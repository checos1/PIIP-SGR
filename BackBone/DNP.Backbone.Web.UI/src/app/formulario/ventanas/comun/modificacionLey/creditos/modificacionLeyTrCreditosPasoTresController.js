(function () {
    ...
    modificacionLeyTrCreditosPasoTresController.$inject = [
        '$scope',
        '$sessionStorage',
        'utilidades',
        '$timeout',
    ...
    function modificacionLeyTrCreditosPasoTresController(
        $scope,
        $sessionStorage,
        utilidades,
        $timeout,
        ...
        function GuardarValores(fuentes) {
            ...
            angular.forEach(fuentes.Creditos, function (series) {
                let valores = {
                    ...
                    NacionCSF: series.origen == 'N' ? series.ValorAcreditarCSFA : 0,
                    NacionSSF: series.origen == 'N' ? series.ValorAcreditarSSFA : 0,
                    Propios: series.origen == 'P' ? series.ValorAcreditarCSFA: 0
                };
...
/*                            ObtenerInformacionPresupuestalDetalle();*/
        ...
        vm.actualizaFila = function (event, fuentes, campo, index) {
            ...
            if (campo === 'ValorAcreditarCSFA') fuentes.Creditos[index].ValorAcreditarCSFA = event.target.value;
            if (campo === 'ValorAcreditarSSFA') fuentes.Creditos[index].ValorAcreditarSSFA = event.target.value;

            angular.forEach(fuentes.Creditos, function (series) {
                TAcreditar += parseFloat(series.ValorAcreditarCSFA) + parseFloat(series.ValorAcreditarSSFA);
            });

            fuentes.TotalAcreditarA = TAcreditar;
    ...
    angular.module('backbone').component('modificacionLeyTrCreditosPasoTres', {

        templateUrl: "src/app/formulario/ventanas/comun/modificacionLey/creditos/modificacionLeyTrCreditosPasoTres.html",
        controller: modificacionLeyTrCreditosPasoTresController,
        controllerAs: "vm",
        bindings: {
            tramiteproyectoid: '@',
            origen: '@',
