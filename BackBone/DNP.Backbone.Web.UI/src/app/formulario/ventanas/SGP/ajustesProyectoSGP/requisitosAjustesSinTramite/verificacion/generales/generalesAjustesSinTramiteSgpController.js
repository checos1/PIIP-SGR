(function () {
    'use strict';
    generalesAjustesSinTramiteSgpController.$inject = [
        'utilidades',
        '$sessionStorage',
    ...
    function generalesAjustesSinTramiteSgpController(
        utilidades,
        $sessionStorage,
        requisitosSinTramiteSgpServicio,
        justificacionCambiosServicio,
        ...
        vm.user = {};
        vm.lang = "es";
        vm.nombreComponente = "sgpviabilidadrequisitosverificacionrequisitossintramitesgp";
        ...
        vm.componentesRefresh = [           
            "sgpviabilidadrequisitosverificacionrequisitossintramitesgp"
        ];
        ...
        function DevolverCuestionarioProyecto() {
            requisitosSinTramiteSgpServicio.ConsultarAccionPorInstancia(vm.idInstancia, vm.idAccion).then(
                function (result) {
                    requisitosSinTramiteSgpServicio.DevolverCuestionarioProyecto(vm.IdNivel, vm.idInstancia, result.data.EstadoAccionPorInstanciaId).then(
                        function (response) {
        ...
        function obtenerPreguntasPersonalizadas(bPin, nivelId, instanciaId, listaRoles) {
            return requisitosSinTramiteSgpServicio.obtenerPreguntasPersonalizadasComponenteSGP(bPin, nivelId, instanciaId, vm.nombreComponente, listaRoles).then(
                function (respuesta) {
                        ...
                        preguntas.ObligaObservacion.forEach(ObligaObservacionValue => {
                            ...
                            if (ObligaObservacionValue.OpcionId == preguntas.Respuesta && tmpJustifica.length <= 5) {
                                if (obsObligatoria != undefined) {
                                    ...
                                    mensaje = "Es necesario que se ingresen todas las justificaciones"
                                    validopregunta = true;
                                }
                            }
                            else {
                ...
                generales.Preguntas.forEach(pregunta => {
                    ...
                    if (pregunta.Respuesta == null) {
                        if (resObligatoria != undefined) {
                            ...
                            mensaje = "Es necesario que se responda todas las preguntas"
                            validopregunta = true;
                        }
                    }
                    else {
        ...
        function Guardar() {
            return requisitosSinTramiteSgpServicio.guardarRespuestasCustomSGP(vm.PreguntasPersonalizadas).then(
                function (response) {
                    if (response.data && response.status === 200) {
                        if (response.data.Exito) {
                            parent.postMessage("cerrarModal", window.location.origin);
                            guardarCapituloModificado(vm.nombreComponente);
                            if (vm.SinPreguntasEspecificas) guardarCapituloModificado('sgpviabilidadrequisitosverificacionrequisitossintramitesgp');
    ...
    angular.module('backbone').component('generalesAjustesSinTramiteSgp', {
        templateUrl: "src/app/formulario/ventanas/SGP/ajustesProyectoSGP/requisitosAjustesSinTramite/verificacion/generales/generalesAjustesSinTramiteSgp.html",
        controller: generalesAjustesSinTramiteSgpController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
            guardadoevent: '&',
