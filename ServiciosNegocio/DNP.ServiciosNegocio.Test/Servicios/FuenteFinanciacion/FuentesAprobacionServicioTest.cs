...
namespace DNP.ServiciosNegocio.Test.Servicios.FuentesProgramarSolicitado
{
    ...
    [TestClass]
    public class FuentesAprobacionServicioTest
    {
        private string BpinBase { get; set; }
        private IFuentesAprobacionServicio _fuentesAprobacionServicio;
        private FuentesAprobacionServicio FuentesAprobacionServicio { get; set; }
        ...
        private IFuentesAprobacionPersistencia FuentesAprobacionPersistencia { get; set; }
        ...
        public void Init()
        {
            ...
            _fuentesAprobacionServicio = contenedor.Resolve<IFuentesAprobacionServicio>();
            FuentesAprobacionPersistencia = UnityContainerExtensions.Resolve<IFuentesAprobacionPersistencia>(contenedor);
            PersistenciaTemporal = contenedor.Resolve<IPersistenciaTemporal>();
            AuditoriaServicio = contenedor.Resolve<IAuditoriaServicios>();
            FuentesAprobacionServicio = new FuentesAprobacionServicio(FuentesAprobacionPersistencia, PersistenciaTemporal, AuditoriaServicio);
            ...
            _mockContext.Setup(mc => mc.uspGetSeguimientoProyectoGR(1, 1, 1, new Guid("418D76AC-F081-4D73-A05A-530CD7C6AFF6"))).Returns(mockFuentes.Object);
            _mockContext.Setup(mc => mc.uspGetPreguntasAprobacionJefePlaneacion(1, 1, 1, new Guid("418D76AC-F081-4D73-A05A-530CD7C6AFF6"))).Returns(mockFuentes.Object);
        ...
        {
            _fuentesAprobacionServicio = new FuentesAprobacionServicio(new FuentesAprobacionPersistencia(_mockContextFactory.Object), new PersistenciaTemporal(_mockContextFactory.Object), new AuditoriaServicios());
            var parametros = new PreguntasSeguimientoProyectoDto
            {
                tramiteId = 1,
                proyectoId = 1,
                tipoTramiteId = 1,
                nivelId = new Guid("418D76AC-F081-4D73-A05A-530CD7C6AFF6"),
                instanciaId = new Guid("4C2E62CD-CEAD-48EF-88C6-A50AB5913716"),
                faseId = new Guid("f562885e-3c75-d1b4-6ebc-4bcebb17ca6c"),
                lstRespuestas = new List<RespuestasSeguimientoProyectoDto>
                {
                    new RespuestasSeguimientoProyectoDto
                    {
                        idPregunta = 1,
                        ProyectoId = 1,
                        idRespuesta = 1,
            ...
            var result = _fuentesAprobacionServicio.ObtenerPreguntasAprobacionRol(parametros);
            Assert.IsNotNull(result);
        }

        [TestMethod]
        public void ObtenerPreguntasAprobacionJefe_Test()
        {
            _fuentesAprobacionServicio = new FuentesAprobacionServicio(new FuentesAprobacionPersistencia(_mockContextFactory.Object), new PersistenciaTemporal(_mockContextFactory.Object), new AuditoriaServicios());
            var parametros = new PreguntasSeguimientoProyectoDto
            {
            ...
            var result = _fuentesAprobacionServicio.ObtenerPreguntasAprobacionJefe(parametros);
            Assert.IsNotNull(result);
        }
        ...
        {
            FuentesAprobacionServicio = new FuentesAprobacionServicio(new FuentesAprobacionPersistencia(_mockContextFactory.Object), new PersistenciaTemporal(_mockContextFactory.Object), new AuditoriaServicios());
            var parametros = new ParametrosConsultaDto();
            FuentesAprobacionServicio.Obtener(parametros);
        }
        ...
        {
            FuentesAprobacionServicio = new FuentesAprobacionServicio(new FuentesAprobacionPersistencia(_mockContextFactory.Object), new PersistenciaTemporal(_mockContextFactory.Object), new AuditoriaServicios());
            var parametros = new ParametrosGuardarDto<PreguntasSeguimientoProyectoDto>();
            var parametroAuditoria = new ParametrosAuditoriaDto()
            {
                Usuario = "Leticia01",
                Ip = "localhost"
            };
            FuentesAprobacionServicio.Guardar(parametros, parametroAuditoria, false);
        }
    }
}
