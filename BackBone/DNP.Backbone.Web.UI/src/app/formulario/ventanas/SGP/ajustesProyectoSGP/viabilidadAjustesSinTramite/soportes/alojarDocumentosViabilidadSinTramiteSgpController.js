(function () {
    ...
    alojarDocumentosViabilidadSinTramiteSgpController.$inject = [
        '$scope',
        '$sessionStorage',
    ];

    function alojarDocumentosViabilidadSinTramiteSgpController(
        $scope,
        $sessionStorage,
    ) {
        var vm = this;
        ...
        vm.nombreComponente = "sgpviabilidadsoportes";
        vm.tipotramiteid = $sessionStorage.tipotramiteid;
        vm.tramiteid = $sessionStorage.tipotramiteid;
        vm.nombreComponenteHijo = "sgpviabilidadsoportesalojararchivossintramitesgp";
        vm.notificacionCambiosCapitulos = null;
        vm.rol = null;

        vm.handlerComponentes = [
            { id: 1, componente: 'sgpviabilidadsoportesalojararchivossintramitesgp', handlerValidacion: null, handlerCambios: null, esValido: true }
        ];
        vm.handlerComponentesChecked = {};

        vm.init = function () {
            var accionViabilidad = $sessionStorage.listadoAccionesTramite.find(f => f.Ventana == 'viabilidadAjustesSinTramiteSgp');
            if (accionViabilidad != null && accionViabilidad.Roles.length > 0) {
                vm.rol = accionViabilidad.Roles[0].IdRol;
            }
        ...
        */
        vm.notificacionValidacionEvent = function (listErrores) {
            ...
            if (erroresList.length > 0) {
                ...
                var idSpanAlertComponent = document.getElementById("alert-" + vm.nombreComponenteHijo);
                if (idSpanAlertComponent != undefined) {
                    idSpanAlertComponent.classList.remove("ico-advertencia");
                }

                var campoObligatorioJustificacion = document.getElementById(vm.nombreComponenteHijo + "-archivo-error");
                if (campoObligatorioJustificacion != undefined) {
                    campoObligatorioJustificacion.innerHTML = "";
                    campoObligatorioJustificacion.classList.add('hidden');
                }

                var campomensajeerror2 = document.getElementById(vm.nombreComponenteHijo + "A" + "-archivo-error");
                if (campomensajeerror2 != undefined) {
                    campomensajeerror2.innerHTML = "";
                    campomensajeerror2.classList.add('hidden');
                }
                var campomensajeerror3 = document.getElementById(vm.nombreComponenteHijo + "B" + "-archivo-error");
    ...
    angular.module('backbone').component('alojarDocumentosViabilidadSinTramiteSgp', {
        templateUrl: "/src/app/formulario/ventanas/SGP/ajustesProyectoSGP/viabilidadAjustesSinTramite/soportes/alojarDocumentosViabilidadSinTramiteSgp.html",
        controller: alojarDocumentosViabilidadSinTramiteSgpController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
