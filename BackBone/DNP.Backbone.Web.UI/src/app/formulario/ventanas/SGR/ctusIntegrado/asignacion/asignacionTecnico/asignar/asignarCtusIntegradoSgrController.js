(function () {
    'use strict';
    asignarCtusIntegradoSgrController.$inject = [
        'utilidades',
        '$sessionStorage',
    ...
    function asignarCtusIntegradoSgrController(
        utilidades,
        $sessionStorage,
        justificacionCambiosServicio,
        transversalSgrServicio,
        ...
        vm.user = {};
        vm.lang = "es";
        vm.nombreComponente = 'sgrctusintegradoasignacionasignar';
        ...
        ctusIntegradoAsignacionSgrServicio.registrarObservador(function (datos) {
        ...
        function ObtenerProyectoCtus(proyectoId, instanciaId) {
            ctusIntegradoAsignacionSgrServicio.SGR_Proyectos_LeerProyectoCtus(proyectoId, instanciaId)
                .then(function (response) {
                    if (response.data != null) {
                        vm.ProyectoCtusId = response.data.id;
                        ValidarUsuarioEncargado(proyectoId, instanciaId);
                    }
                }, function (error) {
        ...
        function ValidarUsuarioEncargado(proyectoId, idInstancia) {
            ctusIntegradoAsignacionSgrServicio.SGR_Proyectos_LeerAsignacionUsuarioEncargado(proyectoId, idInstancia)
                .then(function (response) {
                    if (response.data.length != 0) {
                        ...
                        ctusIntegradoAsignacionSgrServicio.notificarCambio({ usuarioDelegado: 'true' });
                        ctusIntegradoAsignacionSgrServicio.notificarCambio({ reqEntidad: false });
                    }
                    else {
        ...
        function ObtenerUsuariosEntidad(rolId) {
            ...
            ctusIntegradoAsignacionSgrServicio.ObtenerUsuariosEntidadAdscrita(parametros)
                .then(function (response) {
                    if (response.data != null)
            ...
            if (vm.activar == true) {
                ...
                utilidades.mensajeWarning("¿Está seguro de continuar?", function funcionContinuar() {
                    ...
                    if (vm.usuarioEncargado != '')
                        ...
                        ctusIntegradoAsignacionSgrServicio.notificarCambio({ usuarioDelegado: 'false' });
                    }
                    vm.estadoBotonEdit = 'EDITAR';
                    vm.activar = true;
        ...
        vm.guardar = function () {
            if (vm.data.IdUsuarioDNP === null || vm.data.IdUsuarioDNP === undefined || vm.data.IdUsuarioDNP === "") {
                utilidades.mensajeError('El campo funcionario encargado de la elaboración del concepto integrado es obligatorio.', false);
                return;
            }
            ...
            GuardarCtus(dataCtus);
        }

        function GuardarCtus(dataCtus) {
            return ctusIntegradoAsignacionSgrServicio.SGR_CTUS_GuardarAsignacionUsuarioEncargado(dataCtus)
                .then(function (response) {
                    if (response.data.Exito) {
                        guardarCapituloModificado();
                        ctusIntegradoAsignacionSgrServicio.notificarCambio({ guardado: 'true' });
                        utilidades.mensajeSuccess(`Se ha asignado la elaboración de concepto integrado a ${vm.usuarioSeleccionado.NombreUsuario} - ${vm.usuarioSeleccionado.Correo}.`, false, false, false);
                        vm.estadoBotonEdit = 'EDITAR';
                        vm.activar = true;
                        vm.disabledAsig = true;
                    } else {
                        utilidades.mensajeError("Error al guardar el usuario encargado del de concepto integrado.", false);
                    }
                })
                .catch(function () {
                    utilidades.mensajeError('Ocurrió un problema al guardar el usuario encargado del de concepto integrado.');
                });
        }
    ...
    angular.module('backbone').component('asignarCtusIntegradoSgr', {
        templateUrl: "/src/app/formulario/ventanas/SGR/ctusIntegrado/asignacion/asignacionTecnico/asignar/asignarCtusIntegradoSgr.html",
        controller: asignarCtusIntegradoSgrController,
        controllerAs: "vm",
        bindings: {
            guardadoevent: '&',
            notificacionestado: '&',
