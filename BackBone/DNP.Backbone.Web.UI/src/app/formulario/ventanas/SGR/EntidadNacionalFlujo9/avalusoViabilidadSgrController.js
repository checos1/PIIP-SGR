(function () {
    ...
    avalusoViabilidadSgrController.$inject = [
        '$scope',
        'utilidades',
        '$sessionStorage',
    ...
    function avalusoViabilidadSgrController(
        $scope,
        utilidades,
        $sessionStorage,
        utilsValidacionSeccionCapitulosServicio,
        ...
        vm.init = function () {
            ...
            utilsValidacionSeccionCapitulosServicio.getSeccionCapitulo(vm.guiMacroproceso, $sessionStorage.idNivel, $sessionStorage.idFlujoIframe).then(function (respuesta) {
                ...
                var indexobs = respuesta.data.findIndex(p => p.SeccionModificado == 'avaluso');
                vm.SeccionIdViabilidad = respuesta.data[indexobs].SeccionId;
            });

            var accionRequisitos = $sessionStorage.listadoAccionesTramite.find(f => f.Ventana == 'avalusoViabilidadSgr');
        ...
        * */
        vm.handlerComponentes = [
            { id: 1, componente: 'sgrviabilidadavaluso', handlerValidacion: null, handlerCambios: null, esValido: true, errores: null },
            { id: 2, componente: 'sgrviabilidadavalusosoportes', handlerValidacion: null, handlerCambios: null, esValido: true, errores: null }
        ];
        ...
        vm.inicializarComponenteCheck = function (estado) {
            ...
            vm.handlerComponentesChecked = {
                'sgrviabilidadavaluso': true,
                'sgrviabilidadavalusosoportes': true
            };
        }
        ...
        function eventoValidar() {
            ...
            const seccionCap = 'sgrviabilidadavalusosoportes';

            //validar que obtenerErroresviabilidadSgr sea del servico correcto
            avalusoViabilidadSgrServicio.obtenerErroresviabilidadAvalUsoSgr(vm.guiMacroproceso, vm.ProyectoId, vm.IdNivel, vm.idInstancia)
                .then(respuesta => {
                    if (!respuesta.data)
                        return;

                    return validacionArchivosServicio.validarArchivosAdjuntos(
                        respuesta.data, vm.section, vm.IdRol, vm.nivelarchivo, vm.idtipotramitepresupuestal, seccionCap, 'avaluso'
                    );
                })
                .then(respVal => {
                    if (!respVal)
        ...
        //function eventoValidar() {
        //    vm.inicializarComponenteCheck();
        //    avalusoViabilidadSgrServicio.obtenerErroresviabilidadAvalUsoSgr(vm.guiMacroproceso, vm.ProyectoId, vm.IdNivel, vm.idInstancia).then(function (respuesta) {
        ...
        //                        var seccion = 'sgrviabilidadavalusosoportes';
        //                        var capitulo = 'alojararchivo';

        //                        var validacionArchivos = respuesta.data.find(p => p.Seccion == seccion);
        //                        var errorValidacion = JSON.stringify({ [seccion + capitulo]: [{ 'Error': 'SGRAVAL2', 'Descripcion': mensajeError, 'Completo': false }] });
        ...
        vm.visualizarAlerta = function (error, errorObservacion) {
            ...
            else utilidades.mensajeSuccess("Validación realizada satisfactoriamente. Para continuar al siguiente proceso, por favor de clic en siguiente.", false, false, false);
       ...
       */
        vm.showAlertError = function (nombreComponente, esValido) {
            ...
            if (nombreComponente == "sgrviabilidadavaluso")
                nomAlerta = "alert-" + "_" + nombreComponente;
            else
    ...
    angular.module('backbone').component('avalusoViabilidadSgr', {
        templateUrl: "/src/app/formulario/ventanas/SGR/EntidadNacionalFlujo9/avalusoViabilidadSgr.html",
        controller: avalusoViabilidadSgrController,
        controllerAs: "vm",
        bindings: {
            callback: '&'
        }
