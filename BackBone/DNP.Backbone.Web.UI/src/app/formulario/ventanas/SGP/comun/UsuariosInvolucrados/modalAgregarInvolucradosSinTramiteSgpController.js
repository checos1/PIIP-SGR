(function () {
    ...
    angular.module('backbone')
        .controller('modalAgregarInvolucradosSinTramiteSgpController', modalAgregarInvolucradosSinTramiteSgpController);

    modalAgregarInvolucradosSinTramiteSgpController.$inject = [
        '$scope',
        '$uibModalInstance',
        '$sessionStorage',
        'utilidades',
    ...
    function modalAgregarInvolucradosSinTramiteSgpController(
        $scope,
        $uibModalInstance,
        $sessionStorage,
        utilidades,
        ...
        function init() {
            ...
            const componenteRequisitos = vm.involucrado.TipoConceptoViabilidad == 'AjustesConTramiteSGP' ? 'requisitosAjustesConTramiteSgp' : 'requisitosAjustesSinTramiteSgp';
            const componenteViabilidad = vm.involucrado.TipoConceptoViabilidad == 'AjustesConTramiteSGP' ? 'viabilidadAjustesConTramiteSgp' : 'viabilidadAjustesSinTramiteSgp';

            if (vm.involucrado.TipoRolConceptoId === 2) {
                var viabilidadSgp = $sessionStorage.listadoAccionesTramite.find(f => f.Ventana == componenteViabilidad);
                if (viabilidadSgp != null) {
                    vm.IdRol = viabilidadSgp.RolId;
                }

            } else {
                var requisitosSgp = $sessionStorage.listadoAccionesTramite.find(f => f.Ventana == componenteRequisitos);
                if (requisitosSgp != null) {
                    vm.IdRol = requisitosSgp.Roles[0].IdRol;
                }
            }

            usuariosInvolucradosSgpServicio.ObtenerEntidadDestinoResponsableFlujo(vm.IdRol, $sessionStorage.crTypeId, $sessionStorage.idEntidad, vm.involucrado.ProyectoId).then(
