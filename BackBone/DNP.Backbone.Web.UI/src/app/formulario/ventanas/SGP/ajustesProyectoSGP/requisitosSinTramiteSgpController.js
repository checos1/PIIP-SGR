(function () {
    ...
    requisitosSinTramiteSgpController.$inject = [
        '$scope',
        '$location',
        'utilidades',
        '$sessionStorage',
    ...
    function requisitosSinTramiteSgpController(
        $scope,
        $location,
        utilidades,
        $sessionStorage,
        ...
        vm.init = function () {
            ...
            var accionRequisitos = $sessionStorage.listadoAccionesTramite.find(f => f.Ventana == 'requisitosAjustesSinTramiteSgp');
        ...
        * */
        vm.handlerComponentes = [            
            { id: 1, componente: 'sgpviabilidadrequisitosverificacion', handlerValidacion: null, handlerCambios: null, esValido: true, errores: null },
            { id: 2, componente: 'sgpviabilidadrequisitossoportes', handlerValidacion: null, handlerCambios: null, esValido: true, errores: null }
        ];
        ...
        function eventoValidar() {
            ...
            vm.validaCuestionario = false;
            requisitosSinTramiteSgpServicio.obtenerErroresviabilidadSgp(vm.guiMacroproceso, vm.ProyectoId, vm.IdNivel, vm.idInstancia).then(function (respuesta) {

                if (respuesta.data) {

                    //Validar respuestas cuestionario viabilidad
                    let objCodigoErrores = ['SGRERRVIAVB'];
                    ...
                    requisitosSinTramiteSgpServicio.ConsultarAccionPorInstancia(vm.idInstancia, vm.IdAccion).then(function (result) {

                        requisitosSinTramiteSgpServicio.obtenerPreguntasPersonalizadas(vm.Bpin, vm.IdNivel, vm.idInstancia, result.data.EstadoAccionPorInstanciaId).then(function (retorno) {
                                    ...
                                    archivoServicios.obtenerListadoArchivos(param, vm.coleccion).then(function (response) {
                                        ...
                                        if (response === undefined || typeof response === 'string') {
                                            ...
                                            listaTipoArchivosObligatorios.forEach(archivo => {
                                                if (response == null || !response.some(a => (a.metadatos.tipodocumento === archivo.TipoDocumento) && (a.status !== 'Eliminado'))) {
        ...
        vm.visualizarAlerta = function (error, errorObservacion) {
            ...
            if (error) {
                ...
                if (vm.validaCuestionario) {
                    ...
                    utilidades.mensajeSuccess("Usted puede ajustar los campos del formulario para hacer una nueva viabilidad, o devolverlo a verificación de requisitos, o a la MGA.", false, false, false, `Tras la validación, la viabilidad ha sido evaludada  <br /> <span style="color: #A80521">NO FAVORABLE</span>`);
                }
                else {
                    utilidades.mensajeSuccess("Usted puede ajustar los campos del formulario para hacer una nueva viabilidad, enviarlo a firmas y emisión.", false, false, false, `Tras la validación, la viabilidad ha sido evaludada  <br /> <span> VIABLE </span>`);
    ...
    angular.module('backbone').component('requisitosSinTramiteSgp', {
        templateUrl: "/src/app/formulario/ventanas/SGP/ajustesProyectoSGP/requisitosSinTramiteSgp.html",
        controller: requisitosSinTramiteSgpController,
        controllerAs: "vm",
        bindings: {
            callback: '&'
        }
