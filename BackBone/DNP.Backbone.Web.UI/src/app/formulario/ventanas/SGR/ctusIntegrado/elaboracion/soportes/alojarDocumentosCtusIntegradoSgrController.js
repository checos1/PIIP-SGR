(function () {
    ...
    alojarDocumentosCtusIntegradoSgrController.$inject = [
        '$scope',
        '$sessionStorage',
    ];

    function alojarDocumentosCtusIntegradoSgrController(
        $scope,
        $sessionStorage,
    ) {
        var vm = this;
        ...
        vm.nombreComponente = "sgrctusintegradosoportes";
        vm.tipotramiteid = $sessionStorage.tipotramiteid;
        vm.tramiteid = $sessionStorage.tipotramiteid;
        vm.nombreComponenteHijo = "sgrctusintegradosoportesalojararchivo";
        vm.notificacionCambiosCapitulos = null;
        vm.rol = null;

        vm.handlerComponentes = [
            { id: 1, componente: 'sgrctusintegradosoportesalojararchivo', handlerValidacion: null, handlerCambios: null, esValido: true }
        ];
        vm.handlerComponentesChecked = {};

        vm.init = function () {
            var accionViabilidad = $sessionStorage.listadoAccionesTramite.find(f => f.Ventana == 'ElaboracionCormagdalenaSgr' || f.Ventana == 'ElaboracionctusIntegradoSgr');
    ...
    angular.module('backbone').component('alojarDocumentosCtusIntegradoSgr', {
        templateUrl: "/src/app/formulario/ventanas/SGR/ctusIntegrado/elaboracion/soportes/alojarDocumentosCtusIntegradoSgr.html",
        controller: alojarDocumentosCtusIntegradoSgrController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
