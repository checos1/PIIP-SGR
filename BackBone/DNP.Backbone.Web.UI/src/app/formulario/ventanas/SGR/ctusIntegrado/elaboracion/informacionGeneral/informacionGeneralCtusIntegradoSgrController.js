(function () {
    ...
    informacionGeneralCtusIntegradoSgrController.$inject = [
        '$scope',
        '$sessionStorage',
    ];

    function informacionGeneralCtusIntegradoSgrController(
        $scope,
        $sessionStorage,
    ) {
        var vm = this;
        ...
        vm.nombreComponente = "sgrctusintegradoelaboracioninfogeneral";
        vm.tipotramiteid = $sessionStorage.tipotramiteid;
        vm.tramiteid = $sessionStorage.tipotramiteid;
        vm.nombreComponenteHijo = "sgrctusintegradoelaboracioninfogeneralinformacionbasica";
        ...
        vm.handlerComponentes = [
            { id: 1, componente: 'sgrctusintegradoelaboracioninfogeneralinformacionbasica', handlerValidacion: null, handlerCambios: null, esValido: true },
            { id: 2, componente: 'sgrctusintegradoelaboracioninfogeneralconceptospreviosemitidos', handlerValidacion: null, handlerCambios: null, esValido: true },
            { id: 3, componente: 'sgrctusintegradoelaboracioninfogeneralusuariosinvolucrados', handlerValidacion: null, handlerCambios: null, esValido: true }
        ];
       ...
       * */
        ...
        vm.inicializarComponenteCheck = function () {
            vm.handlerComponentesChecked = {
                'sgrctusintegradoelaboracioninfogeneralinformacionbasica': true,
                'sgrctusintegradoelaboracioninfogeneralconceptospreviosemitidos': true,
                'sgrctusintegradoelaboracioninfogeneralusuariosinvolucrados': true
            };
        }
        ...
        */
        vm.notificacionValidacionEvent = function (listErrores) {
            ...
            if (erroresList.length > 0) {
                ...
                for (var i = 0; i < vm.handlerComponentes.length; i++) {
                    var campomensajeerror = document.getElementById("alert-" + vm.handlerComponentes[i].componente);
                ...
                var errorElementsArray = Array.from(errorElements);

                errorElementsArray.forEach(function (element) {
                    element.classList.remove('ico-advertencia');
                });
            }
        }
    ...
    angular.module('backbone').component('informacionGeneralCtusIntegradoSgr', {
        templateUrl: "/src/app/formulario/ventanas/SGR/ctusIntegrado/elaboracion/informacionGeneral/informacionGeneralCtusIntegradoSgr.html",
        controller: informacionGeneralCtusIntegradoSgrController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
