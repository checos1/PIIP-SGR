(function () {
	...
	ResumenFocalizacionController.$inject = ['$scope', '$sessionStorage', '$uibModal', 'utilidades',
		'constantesBackbone', '$timeout', 'focalizacionAjustesServicio', 'justificacionCambiosServicio'
	];

	function ResumenFocalizacionController(
		$scope,
		$sessionStorage,
		$uibModal,
		utilidades,
		...
		vm.nombreComponente = "focalizacionpolresumendefocali";
		...
		function init() {
			...
			//ObtenerSeccionCapitulo();//Esto se llama al momento de guardar la información
		...
		vm.ConsultarPoliticasCategoriasIndicadores = function (bpin) {
			...
			return focalizacionAjustesServicio.ConsultarPoliticasCategoriasIndicadores($sessionStorage.idInstancia, usuarioDNP, idInstancia).then(
				function (respuesta) {
		...
		vm.obtenerPoliticasTransversalesCategorias = function (bpin) {
			...
			return focalizacionAjustesServicio.obtenerPoliticasTransversalesCategorias(parametroConsulta, usuarioDNP, idInstancia).then(
				function (respuesta) {
					if (respuesta.data != null && respuesta.data != "") {
		...
		vm.GuardarAjustes = function (politicaId, categoriaId, fuente, producto, localizacionId, focalizacionAjustada, localizacion) {
			var a = 0;
			utilidades.mensajeWarning("Los valores de las columnas 'En Ajuste$', 'Meta Categoria', 'Personas Categoria' y 'Meta Indicador Secundario' van a ser guardados, ¿desea continuar?", function funcionContinuar() {
				...
				GuardarArchivoFocalizacion(false, 0, localizacion);
				localizacion.HabilitaEditarLocalizador = false;
				vm.listaDetalleLocalizacion.HabilitaEditarLocalizador = false;
			}, function funcionCancelar(reason) {
			});
		...
		function GuardarArchivoFocalizacion(valorescero, cargamasiva, localizacion) {
			if (cargamasiva)
				vm.activarControles('inicio');
			return focalizacionAjustesServicio.guardarFocalizacionCategorias(vm.FocalizacionArchivo, vm.idUsuario).then(function (response) {
				//var respuesta = jQuery.parseJSON(response.data)

				//angular.forEach(localizacion.FocalizacionAjustada, function (series) {
				//	series.EnAjusteF = ConvertirNumero2decimales(series.EnAjuste);
				//	series.MetaCategoriaF = ConvertirNumero2decimales(series.MetaCategoria);
				//	series.PersonasCategoriaF = ConvertirNumero2decimales(series.PersonasCategoria);
				//	series.MetaIndicadorSecundarioF = ConvertirNumero2decimales(series.MetaIndicadorSecundario);
				//});
														...
														if (productos.ProductoId == vm.FocalizacionArchivo[0].ProductoId) {
															productos.Localizaciones.forEach(localizaciones => {
																if (localizaciones.LocalizacionId == vm.FocalizacionArchivo[0].LocalizacionId) {
																	if (localizaciones.FocalizacionAjustada !== undefined) {
																		localizaciones.FocalizacionAjustada.forEach(focalizacionajustada => {
		...
		function valoresEnCero(politicaId, categoriaId, fuenteId, productoId, localizacionId, focalizacionAjustada, localiizacion) {
			...
			utilidades.mensajeWarning("Los valores de las columnas de la tabla 'Focalización ajustada' quedarán en ceros ¿esta seguro de continuar?", function funcionContinuar() {
				...
				GuardarArchivoFocalizacion(true, 0, localiizacion);
	...
	angular.module('backbone').component('resumenFocalizacion', {
		templateUrl: "src/app/formulario/ventanas/ajustes/componentes/focalizacion/ResumenFocalizacion/ResumenFocalizacion.html",
		controller: ResumenFocalizacionController,
		controllerAs: "vm",
		bindings: {
			callback: '&',
