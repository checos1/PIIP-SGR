(function () {
    'use strict';
    angular.module('backbone').controller('tramitesController', tramitesController);
    ...
    function tramitesController(
        ...
        vm.IdTramitesTrasladoSgp = "";
        vm.IdTramitesAdicionSgp = "";
        vm.NombrePrimerPasoTramitesSgp = "";
        vm.listaTramitesSgp = [];
        ...
        function cargarEntidades(idObjeto) {
            ...
            return servicioPanelPrincipal.obtenerTramites(vm.peticionObtenerInbox, vm.tramiteFiltro, columnasVisibles).then(
                function (respuesta) {
                    ...
                    // }                    
                    transversalSgpServicio.SGPTransversalLeerParametro("IdTramitesSgp")
                        .then(function (respuestaParametro) {
                            if (respuestaParametro.data) {
                                vm.listaTramitesSgp = respuestaParametro.data.Valor.split('|');;                                
                                vm.listaTramitesSgp.forEach(tramId => {
                                    tramId.split(',');                                        
                                    vm.IdTramitesTrasladoSgp = tramId.split(',')[0];
                                    vm.IdTramitesAdicionSgp = tramId.split(',')[1];
                                });                                
                            }
                        });
                    transversalSgpServicio.SGPTransversalLeerParametro("NombrePrimerPasoTramitesSgp")
                          .then(function (respuestaParametroSgp) {
                                if (respuestaParametroSgp.data) {
                                    vm.NombrePrimerPasoTramitesSgp = respuestaParametroSgp.data.Valor;
                                }
                        });
                            ...
                            entidad.GrupoTramites.forEach(tramite => {
                                ...
                                tramite.ListaTramites.forEach(instancia => {
                                    vm.listaTramitesCompleta.push({
                                        ...
                                        permiteEliminar: (instancia.TipoTramiteId == vm.IdTramitesTrasladoSgp && instancia.NombreAccion == vm.NombrePrimerPasoTramitesSgp) || (instancia.TipoTramiteId == vm.IdTramitesAdicionSgp && instancia.NombreAccion == vm.NombrePrimerPasoTramitesSgp) ? 1 : 0
        ...
        function consultarAccion(idInstancia, nombreProyecto, idObjetoNegocio, idEntidad, nombreFlujo, row) {
            ...
            if (nombreFlujo !== null) {
                ...
                $sessionStorage.flujoSeleccionado = row.IdFlujo;
            }
            $timeout(function () {
        ...
        function guardarSolicitud() {
            ...
            var data = vm.tramites.find(t => t.id === vm.objSolicitud.codTramite);
            if (data == null) {
                validacionNovigenciaFutura();
                return;
            }
            else if (data.alias == 'VFO' || data.alias == 'VFE'  || data.alias == 'VF') {
                var anioInicial = '';

                if (data.alias == 'VFO') {
                    anioInicial = document.getElementById("anioInicialIdVFO").value;
                }
                if (data.alias == 'VFE') {
                    anioInicial = document.getElementById("anioInicialIdVFE").value;
                }

                var anioFinal = '';

                if (data.alias == 'VFO') {
                    anioFinal = document.getElementById("anioFinalIdVFO").value;
                }
                if (data.alias == 'VFE') {
                    anioFinal = document.getElementById("anioFinalIdVFE").value;
                }

                if (!vm.objSolicitud.codTramite || !vm.objSolicitud.codEntidade || !vm.objSolicitud.objeto || !vm.objSolicitud.anioFinal || !vm.objSolicitud.anioInicial || (data.alias == 'VFO' && !vm.tipoProceso) || !vm.app) {
                    if (!vm.objSolicitud.codTramite) {
                        document.getElementById("tipoTramiteError").innerHTML = "<span class='d-inline-block ico-advertencia'></span><span>  Seleccione una opción.</span>";
                        document.getElementById("tipoTramiteError").classList.remove('hidden');
                    }
                    ...
                    if (!vm.objSolicitud.objeto) {
                        if (data.alias == 'VFE') {
                            document.getElementById("objetoErrorVFE").innerHTML = "<span class='d-inline-block ico-advertencia'></span><span>  Ingrese el objeto de la contratación.</span>";
                            document.getElementById("objetoErrorVFE").classList.remove('hidden');
                        }
                        if (data.alias == 'VFO') {
                            document.getElementById("objetoErrorVFO").innerHTML = "<span class='d-inline-block ico-advertencia'></span><span>  Ingrese el objeto de la contratación.</span>";
                            document.getElementById("objetoErrorVFO").classList.remove('hidden');
                        }
                    }
                    if (!vm.objSolicitud.anioFinal) {
                        if (data.alias == 'VFE') {
                            document.getElementById("anioFinalErrorVFE").innerHTML = "<span class='d-inline-block ico-advertencia'></span><span>  Seleccione una fecha.</span>";
                            document.getElementById("anioFinalErrorVFE").classList.remove('hidden');
                        }
                        if (data.alias == 'VFO') {
                            document.getElementById("anioFinalErrorVFO").innerHTML = "<span class='d-inline-block ico-advertencia'></span><span>  Seleccione una fecha.</span>";
                            document.getElementById("anioFinalErrorVFO").classList.remove('hidden');
                        }
                    }
                    if (!vm.objSolicitud.anioInicial) {
                        if (data.alias == 'VFE') {
                            document.getElementById("anioInicialErrorVFE").innerHTML = "<span class='d-inline-block ico-advertencia'></span><span>  Seleccione una fecha.</span>";
                            document.getElementById("anioInicialErrorVFE").classList.remove('hidden');
                        }
                        if (data.alias == 'VFO') {
                            document.getElementById("anioInicialErrorVFO").innerHTML = "<span class='d-inline-block ico-advertencia'></span><span>  Seleccione una fecha.</span>";
                            document.getElementById("anioInicialErrorVFO").classList.remove('hidden');
                        }
                    }
                    if (data.alias == 'VFO' && !vm.tipoProceso) {
                        document.getElementById("tipoProcesoError").innerHTML = "<span class='d-inline-block ico-advertencia'></span><span>  Seleccione una opción.</span>";
                        document.getElementById("tipoProcesoError").classList.remove('hidden');
                    }
                    if (data.alias == 'VFO' && !vm.app) {
                        document.getElementById("appErrorVFO").innerHTML = "<span class='d-inline-block ico-advertencia'></span><span>  Seleccione una opción.</span>";
                        document.getElementById("appErrorVFO").classList.remove('hidden');
                    }
                    if (data.alias == 'VFE' && !vm.app) {
                        document.getElementById("appErrorVFE").innerHTML = "<span class='d-inline-block ico-advertencia'></span><span>  Seleccione una opción.</span>";
                        document.getElementById("appErrorVFE").classList.remove('hidden');
                    }
                    if (new Date(anioInicial) > new Date(anioFinal)) {
                        utilidades.mensajeError('La fecha de inicio del trámite no puede ser mayor a la fecha final del trámite. Por favor verifique', false, 'Hay datos que presentan inconsistencias');
                        return;
                    }
                    utilidades.mensajeError('Revise los campos señalados y valide nuevamente', false, 'Hay datos que presentan inconsistencias');
                    return;
                ...
                descripcion = vm.objSolicitud.objeto;
            }
            else {
        ...
        function detenerTramite(tramite) {
            ...
            if (cumpleRol.length > 0) {
                if (!tramite.NombreAccion.startsWith(vm.NombrePrimerPasoTramitesSgp)) {
                    utilidades.mensajeError("El trámite No se encuentra en el Paso 1 (Presuepuesto preliminar), por lo tanto, no se puede eliminar.");
                }
                else {
                    utilidades.mensajeWarning("¿Desea eliminar el trámite " + tramite.IdObjetoNegocio + "? <br>Si elimina el trámite, se eliminará toda la información que se ha diligenciado.", function funcionContinuar() {
                        ...
                        servicioPanelPrincipal.detenerInstancia(vm.peticionObtenerInbox, tramite.IdInstancia).then(
                            function (respuesta) {                              
                                mostrarMensajeRespuesta();
                                if (respuesta.data) {
                                    if (respuesta.data.CreadoPor != vm.peticionObtenerInbox.IdUsuario)
                                        utilidades.mensajeError("Esta acción solo puede ser ejecutada por el usuario que creo el trámite, segun sea el caso.");
                                }
                                else {
                                    cargarEntidades();
            ...
            //else { //ROL ANALISTA
            //    var cumpleRolAna = $sessionStorage.usuario.roles.filter(x => x.Nombre.includes("Analista") == true);
            //    if (cumpleRolAna.length > 0) {
            //        if (!tramite.NombreAccion.startsWith('1.')) {
            //            utilidades.mensajeError("El trámite No se encuentra en el Paso 1 (Analista), por lo tanto, no se puede eliminar.");
            //        }
            //        else {
            //            utilidades.mensajeWarning("¿Desea eliminar el trámite " + tramite.IdObjetoNegocio + "? <br>Si elimina el trámite, se eliminará toda la información que se ha diligenciado.", function funcionContinuar() {
            ...
            //        utilidades.mensajeError("Esta acción solo puede ser ejecutada por el rol de Presupuesto Preliminar o Analista, segun sea el caso.");
            //    }
            //}
        }
    ...
    angular.module('backbone')
        .component('tramites', {
            templateUrl: "/src/app/panelPrincial/componentes/inbox/tramites.template.html",
