namespace DNP.EncabezadoPie.Web.API.Controllers
{
    ...
    public class DefinirAlcanceController : ApiController
    {
        private readonly IDefinirAlcanceServicio _definirAlcanceServicio;
        private readonly IAutorizacionUtilidades _autorizacionUtilidades;

        public DefinirAlcanceController(IDefinirAlcanceServicio definirAlcanceServicio, IAutorizacionUtilidades autorizacionUtilidades)
        {
            _definirAlcanceServicio = definirAlcanceServicio;
            _autorizacionUtilidades = autorizacionUtilidades;
        }

        [Route("api/DNP_SN_Seg_PlaEjec_DefinirAlcanceDto")]
        [SwaggerResponse(HttpStatusCode.OK, "Retorna listado de Priorizacion Recurso", typeof(AlcanceDto))]
        [HttpGet]
        public async Task<IHttpActionResult> Consultar(string bpin)
        {
            var respuestaAutorizacion = _autorizacionUtilidades.ValidarUsuario(RequestContext.Principal.Identity.Name,
                                                                               ...
                                                                               ConfigurationManager.AppSettings["consultarDefinirAlcance"]).Result;

            if (!respuestaAutorizacion.IsSuccessStatusCode) return ResponseMessage(respuestaAutorizacion);

            ValidacionParametro(bpin, Request);

            var result = await Task.Run(() => _definirAlcanceServicio.ObtenerAlcance(
        ...
        [Route("api/DNP_SN_Seg_PlaEjec_DefinirAlcanceDto/Preview")]
        [SwaggerResponse(HttpStatusCode.OK, "Retorna listado de Priorizacion Recursos priview", typeof(AlcanceDto))]
        [HttpGet]
        public async Task<IHttpActionResult> Preview()
        {
            try
            {
                var respuestaAutorizacion = _autorizacionUtilidades.ValidarUsuario(RequestContext.Principal.Identity.Name,
                                                                                   RequestContext.Principal.Identity.GetHashCode().ToString(),
                                                                                   ConfigurationManager.AppSettings["idAdministracionEnAutorizaciones"],
                                                                                   ConfigurationManager.AppSettings["previewDefinirAlcance"]).Result;

                if (!respuestaAutorizacion.IsSuccessStatusCode) return ResponseMessage(respuestaAutorizacion);


                var result = await Task.Run(() => _definirAlcanceServicio.ObtenerAlcancePreview());
                return Ok(result);
            }
