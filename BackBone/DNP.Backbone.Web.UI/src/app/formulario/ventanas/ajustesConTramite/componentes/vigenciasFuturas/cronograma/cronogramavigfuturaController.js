(function () {
	...
	cronogramavigfuturaController.$inject = [
		'$sessionStorage',
	...
	function cronogramavigfuturaController(
		$sessionStorage,
		cronogramavigfuturaServicio,
		utilidades,
		$timeout
		...
		this.nombreComponente = "ajustesvigenciasfuturasCronograma";
		...
		function ObtenerModalidadContratacion() {
			...
			return cronogramavigfuturaServicio.ObtenerModalidadContratacion()
				.then((respuesta) => {
		...
		this.init = () => {
			...
			// this.notificacionvalidacion({ handler: vm.notificacionValidacionPadre, nombreComponente: vm.nombreComponente, esValido: true });
		};

		this.seleccionarModalidad = (id) => {
			document.getElementById('vModalidad').value = id;
		...
		this.descargarPlantilla = () => {
			...
			if (this.actividadesPrecontractuales.length == 0) {
				utilidades.mensajeError("No hay actividades precontractuales disponibles no se puede descargar la plantilla");
				return;
			}
			var dataPrecontractual = this.actividadesPrecontractuales.map(x => ({ id: x.ActividadPreContractualId, cronogramaId: x.CronogramaId, Actividad: x.Actividad, FechaInicial: moment(x.FechaInicial).format('YYYY/MM/DD HH:mm:ss'), FechaFinal: moment(x.FechaFinal).format('YYYY/MM/DD') }));
			...
			XLSX.writeFile(libro, "ActividadesPre-Contractuales.xlsx");
		}
				...
				try {
					...
					if (file) {
						...
						fileReader.onload = (e) => {
							...
							var precontractuales = XLSX.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]], { raw: false, cellText: false, cellDates: true });
							...
							if (workbook.SheetNames[0] !== 'PreContractual' || workbook.SheetNames[1] !== 'Contractual') {
								console.log('Error estructura: hojas no coinciden');
								reject('Error estructura');
							} else {
								const sheets = { precontractuales, contractuales };
								resolve(sheets);
							}
	...
	angular.module('backbone').component('cronogramavigfuturas', {
		templateUrl: "src/app/formulario/ventanas/ajustesConTramite/componentes/vigenciasFuturas/cronograma/cronogramavigfutura.html",
		controller: cronogramavigfuturaController,
		bindings: {},
	});

	angular.module('backbone').directive('input', function () {
		return {
			restrict: 'E',
			scope: {
				ngModel: '=',
