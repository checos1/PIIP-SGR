(function () {
    ...
    viabilidadVotosSgrController.$inject = [
        '$scope',
        'utilidades',
        '$sessionStorage',
        'viabilidadVotosSgrServicio',
    ...
    function viabilidadVotosSgrController(
        $scope,
        utilidades,
        $sessionStorage,
        viabilidadVotosSgrServicio,
        ...
        //vm.validarDocumentosObligatorios = validarDocumentosObligatorios;

        vm.MostrarMensajeCTUSPendiente = false;
        ...
        vm.init = function () {
            ...
            utilsValidacionSeccionCapitulosServicio.getSeccionCapitulo(vm.guiMacroproceso, $sessionStorage.idNivel, $sessionStorage.idFlujoIframe).then(function (respuesta) {
                ...
                var indexobs = respuesta.data.findIndex(p => p.SeccionModificado == 'sgrocadpazviabilidad');
                vm.SeccionIdViabilidad = respuesta.data[indexobs].SeccionId;
            });

            transversalSgrServicio.SGR_Transversal_LeerParametro("MensajeProyectoVOCADNotificacionSGR")
            ...
            transversalSgrServicio.SGR_Transversal_LeerParametro("MensajeProyectoVOCADSGR")
                .then(function (respuestaParametro) {
                    if (respuestaParametro.data) {
            ...
            transversalSgrServicio.SGR_Transversal_LeerParametro("MensajeProyectoVDetOCADSGR")
                .then(function (respuestaParametro) {
                    if (respuestaParametro.data) {
            ...
            viabilidadVotosSgrServicio.validarInstanciaCTUSNoFinalizada(vm.ProyectoId).then(
                function (respuesta) {
                    ...
                    if (respuesta.data !== '') {
                        ...
                        if (typeof respuestaJson.InstanciaCTUSNoFinalizada !== "undefined") {
                            vm.MostrarMensajeCTUSPendiente = respuestaJson.InstanciaCTUSNoFinalizada;

                            if (vm.MostrarMensajeCTUSPendiente) {
        ...
        * */
        vm.handlerComponentes = [
            { id: 1, componente: 'sgrviabilidad', handlerValidacion: null, handlerCambios: null, esValido: true, errores: null, handlerInicio: null },
            { id: 2, componente: 'sgrviabilidadsoportes', handlerValidacion: null, handlerCambios: null, esValido: true, errores: null }
        ];
        ...
        function eventoValidar() {
            ...
            //validar que obtenerErroresviabilidadOcadPazSgr sea del servico correcto
            viabilidadVotosSgrServicio.obtenerErroresviabilidadOcadPazSgr(vm.guiMacroproceso, vm.ProyectoId, vm.IdNivel, vm.idInstancia)
                .then(respuesta => {
                    if (!respuesta.data)
                        return;
                ...
                .then(respVal => {
                    ...
                    var indexobs = respVal.findIndex(p => p.Capitulo == 'observaciones');
                    var errorObservacion = false;

                    if (indexobs != -1)
                        errorObservacion = respVal[indexobs].Errores == null ? false : true;

                    var findErrors = respVal.findIndex(p => p.Errores != null && p.Seccion != '');
                    var error = respVal.findIndex(p => p.Errores != null && p.Seccion != '');
                    var erroresCumple = respVal.filter(p => p.Errores != null && p.Seccion == '');
                    var erroresJson = erroresCumple.length == 0 ? [] : JSON.parse(erroresCumple[0].Errores);

                    if (error == -1 && (vm.visualizarCumple || !vm.visualizarCumple)) {
                        ...
                        var ocultarDevolver = vm.cumple === 'v';
        ...
        //function eventoValidar() {
        //    vm.inicializarComponenteCheck();
        //    viabilidadVotosSgrServicio.obtenerErroresviabilidadOcadPazSgr(vm.guiMacroproceso, vm.ProyectoId, vm.IdNivel, vm.idInstancia).then(function (respuesta) {
    ...
    angular.module('backbone').component('viabilidadVotosSgr', {
        templateUrl: "src/app/formulario/ventanas/SGR/ctei/viabilidadVotosSgr.html",
        controller: viabilidadVotosSgrController,
        controllerAs: "vm",
        bindings: {
            callback: '&'
        }
