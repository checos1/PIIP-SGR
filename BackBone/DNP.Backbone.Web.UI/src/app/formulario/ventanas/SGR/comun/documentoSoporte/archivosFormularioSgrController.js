(function () {
    ...
    archivosFormularioSgrController.$inject = [
        '$scope',
    ...
    function archivosFormularioSgrController(
        $scope,
        servicioFichasProyectos,
        $sessionStorage,
        utilidades,
        ...
        function consultarTipoDocumento() {
            ...
            archivoServicios.obtenerListaTipoDocumentoSoportePorRol(tipoTramiteId, "A", idTramite, vm.IdNivel, vm.IdInstancia, vm.IdAccion).then(function (resultado) {
        ...
        vm.mostrarBotonPdf = function () {
            var mostrar = false;
            //var generaFichaFlujoIdVerReqSGR = utilidades.obtenerParametroTransversal('GeneraFichaFlujoIdVerReqSGR');
            //var verPfsgeneraFlujoIdVerReqSGR1 = generaFichaFlujoIdVerReqSGR.split(',').some(e => e === $sessionStorage.idFlujoIframe);
            //var verPfsgeneraFlujoIdVerReqSGR2 = generaFichaFlujoIdVerReqSGR.includes($sessionStorage.idFlujoIframe);

            if (vm.nombreComponente == 'sgrviabilidadrequisitossoportesalojararchivo') {
                if (vm.puedeGenerarverReq) {
                    return true;
                }
            }
            if (vm.nombreComponente == "sgrviabilidadsoportesalojararchivo") {
                if (vm.puedeGenerarverReq) {
                    return false;
                }
                return true;
    ...
    angular.module('backbone').component('archivosFormularioSgr', {
        restrict: 'E',
        transclude: true,
        scope: {
            modelo: '='
        },
        templateUrl: "src/app/formulario/ventanas/SGR/comun/documentoSoporte/archivosFormularioSgr.html",
        controller: archivosFormularioSgrController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
