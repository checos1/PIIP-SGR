(function () {
    ...
    productosPrPasoTresController.$inject = ['$scope', '$sessionStorage', 'comunesServicio'];

    function productosPrPasoTresController(
        $scope,
        $sessionStorage,
        comunesServicio
    ) {
        var vm = this;
        vm.lang = "es";
        vm.nombreComponente = "productos";
        vm.nombreComponenteHijo = "productosproductos";
        vm.notificacionCambiosCapitulos = null;

        vm.handlerComponentes = [
            { id: 1, componente: 'productosproductos', handlerValidacion: null, handlerCambios: null, esValido: true }
        ];
        ...
        function validarVariables() {
            if (vm.IdNivel !== undefined && vm.IdEntidad !== undefined && vm.IdSeccionCapitulo !== undefined)
                vm.verificacalendario({ idEntidad: vm.IdEntidad, idNivel: vm.IdNivel, idSeccionCapitulo: vm.IdSeccionCapitulo, seccion: 'productos' });
        }
    ...
    angular.module('backbone').component('productosPrPasoTres', {
        templateUrl: "/src/app/formulario/ventanas/tramiteProgramacionRecursos/componentes/productosPr/productosPrPasoTres.html",
        controller: productosPrPasoTresController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
