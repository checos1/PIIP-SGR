(function () {
    'use strict';
    tecnicoEspecificasController.$inject = [
        'utilidades',
        '$sessionStorage',
        'viabilidadServicio',
    ...
    function tecnicoEspecificasController(
        utilidades,
        $sessionStorage,
        viabilidadServicio,
        justificacionCambiosServicio
        ...
        vm.user = {};
        vm.lang = "es";
        vm.nombreComponente = "viabilidadtecnicoespecificas";
        ...
        vm.BtnTematica = function (campo) {
            var variable = $("#imgTE" + campo)[0].outerHTML;
            if (variable.includes('Img/btnMas.svg')) {
                $("#imgTE" + campo).attr('src', 'Img/btnMenos.svg');
            }
            else {
                $("#imgTE" + campo).attr('src', 'Img/btnMas.svg');
            }
        }
        ...
        function buscarEspecificas() {
            ...
            vm.PreguntasPersonalizadas.PreguntasEspecificas.forEach(especificas => {
                especificas.Preguntas.forEach(preguntas => {
                    ...
                    if (preguntas.IdPregunta.toString() === vm.codigoE) {
                        $("#TematicaIdTE" + especificas.OrdenTematica).removeClass("collapse");
                        $("#u9_imgTE" + especificas.OrdenTematica).removeAttr("style");
            ...
            if (contador > 0) {
                resultado = contador.toString() + " resultados en " + vm.Fase + " técnica (Específicas). Los desplegables con el marcador <img class='imgViabilidad' src='Img/u9.svg'> contienen alguna coincidencia.";
            }

            $("#labelResultadoTE").html(resultado);
        }
        ...
        function limpiarBuscarE(limpiarCampos) {
            ...
            $("#labelResultadoTE").html("");

            vm.PreguntasPersonalizadas.PreguntasEspecificas.forEach(especificas => {
                especificas.Preguntas.forEach(preguntas => {
                    $("#TematicaIdTE" + especificas.OrdenTematica).addClass("collapse");
                    $("#u9_imgTE" + especificas.OrdenTematica).hide();
                    $("#labelIdT" + preguntas.IdPregunta).html(preguntas.Pregunta);
                });
            });
        }
        ...
        vm.ActivarEditar = function () {
            if (vm.activar == true) {
                $("#EditarTE").html("CANCELAR");
                vm.activar = false;
            }
            else {
                $("#EditarTE").html("EDITAR");
                vm.activar = true;
            }
        }
    ...
    angular.module('backbone').component('tecnicoEspecificas', {
        templateUrl: "src/app/formulario/ventanas/viabilidad/componentes/preguntasTecnico/especificas/tecnicoEspecificas.html",
        controller: tecnicoEspecificasController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
            guardadoevent: '&',
