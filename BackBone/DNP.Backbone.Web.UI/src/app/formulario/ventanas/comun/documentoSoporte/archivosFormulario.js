(function () {
    ...
    function archivosFormulario(
        ...
        function consultarTipoDocumento() {
            ...
            if ((vm.rol === undefined || vm.rol === "") || vm.rol == '00000000-0000-0000-0000-000000000000') {
                ...
                archivoServicios.obtenerTipoDocumentoTramitePorRol(vm.tipotramiteid, vm.rol, vm.tramiteid, 55).then(function (resultado) {
        ...
        function cargaTipoArchivosObligatorios(lista) {
            ...
            if (vm.nombrecomponentepaso == 'selecionarvigenciafuturavaloresutilizados') {
                vm.listaTipoArchivos = vm.listaTipoArchivos.filter((item) => item.TipoDocumento == 'Oficio de valores utilizados por la entidad');
            }
        }
        ...
        function consultarArchivosTramite() {
            ...
            archivoServicios.obtenerListadoArchivos(param, vm.modelo.coleccion).then(function (response) {
                ...
                if (response === undefined || typeof response === 'string') {
                    ...
                    if (vm.listadocumentosObligatorios.length == vm.listaArchivos.length) $sessionStorage.sessionDocumentos = 100;
                    ...
                    if ($sessionStorage.sessionDocumentos == 0 && vm.nombrecomponentepaso == 'selecionarvigenciafuturavaloresutilizados') {
                        vm.listaArchivos = vm.listaArchivos.filter((item) => item.tipoDocumento == 'Oficio de valores utilizados por la entidad');
                        if (vm.listaArchivos.length > 0) {
                            //$sessionStorage.sessionDocumentos = 100;
                    ...
                    if ($sessionStorage.sessionDocumentos == 50 && vm.nombrecomponentepaso == 'selecionarvigenciafuturavaloresutilizados') {
                        vm.listaArchivos = vm.listaArchivos.filter((item) => item.tipoDocumento == 'Oficio de valores utilizados por la entidad');
                        if (vm.listaArchivos.length > 0) {
                            $sessionStorage.sessionDocumentos = 100;
                            $sessionStorage.DescripcionAccionNivel = 1;
    ...
    angular.module('backbone').component('archivosFormulario', {
        restrict: 'E',
        transclude: true,
        scope: {
            modelo: '='
        },
        templateUrl: "src/app/formulario/ventanas/comun/documentoSoporte/archivosFormulario.html",
        controller: archivosFormulario,
        controllerAs: "vm",
        bindings: {
            callback: '&',
