(function () {
    'use strict';
    giroReconocimientoSgrController.$inject = [
        'utilidades',
        '$sessionStorage',
    ...
    function giroReconocimientoSgrController(
        utilidades,
        $sessionStorage,
        justificacionCambiosServicio,
        designarEjecutorSgrServicio
        ...
        vm.user = {};
        vm.lang = "es";
        vm.nombreComponente = 'sgrejecutordesignacioncostosgiroreconocimiento';
        ...
        vm.ReconocimientoCostosENChange = function () {
            if (vm.aplicaCostos == false) {
                vm.disabledEdt = true;
                restablecerBusqueda();
            }
            else {
        ...
        function ConsultaEjecutoresAsociados() {
            designarEjecutorSgrServicio.ObtenerRespuestaEjecutorSGR('AplicaReconCostos', vm.proyectoId)
                .then(function (response) {
                    const aplicaReconCostos = response.data;

                    if (aplicaReconCostos) {
                        ConsultarEntidad();
                    } else {
                        $("#ddlEntidadGiro").empty();
                        vm.listaEjecutoresAsociados = null;
                        vm.dsblBtn = false;
                        vm.aplicaCostos = null;
                        vm.tieneEjecutor = false;
                        designarEjecutorSgrServicio.notificarCambio({ recCostos: false });
                    }
                })
                .catch(function (error) {
                    utilidades.mensajeError('Error en el capitulo "Entidad encargada de ordenar el giro del reconocimiento" - servicio "ObtenerRespuestaEjecutorSGR - AplicaReconCostos".');
                    console.error('Error en el capitulo "Entidad encargada de ordenar el giro del reconocimiento" - servicio "ObtenerRespuestaEjecutorSGR - AplicaReconCostos" - Error:', error);
                });
        }

        function ConsultarEntidad() {
            designarEjecutorSgrServicio.ObtenerRespuestaEjecutorSGR('EntidadOrdenaGiro', vm.proyectoId)
                .then(function (response) {
                    const entidadOrdenaGiro = response.data;

                    if (entidadOrdenaGiro) {
                        designarEjecutorSgrServicio.ObtenerEjecutores('', '', entidadOrdenaGiro)
                            .then(function (ejecutoresResponse) {
                                const ejecutores = ejecutoresResponse.data;

                                if (Array.isArray(ejecutores) && ejecutores.length > 0) {
                                    const ejecutor = ejecutores[0];
                            ...
                            .catch(function (error) {
                                utilidades.mensajeError('Error en el capitulo "Entidad encargada de ordenar el giro del reconocimiento" - servicio "ObtenerEjecutores".');
                                console.error('Error en el capitulo "Entidad encargada de ordenar el giro del reconocimiento" - servicio "ObtenerEjecutores" - Error:', error);
                            });

                        vm.dsblBtn = true;
                        vm.aplicaCostos = true;
                        vm.tieneEjecutor = vm.listaEjecutoresAsociados != null && vm.listaEjecutoresAsociados.size > 0;
                        designarEjecutorSgrServicio.notificarCambio({ recCostos: true });
                    } else {
                        vm.tieneEjecutor = false;
                        designarEjecutorSgrServicio.notificarCambio({ recCostos: false });
                        ...
                        if (!vm.disabled) {
                            ...
                            vm.listaEjecutoresAsociados = null;
                            vm.dsblBtn = false;
                            vm.aplicaCostos = false;
                        }
                    }
                })
                .catch(function (error) {
                    utilidades.mensajeError('Error en el capitulo "Entidad encargada de ordenar el giro del reconocimiento" - servicio "ObtenerRespuestaEjecutorSGR - EntidadOrdenaGiro".');
                    console.error('Error en el capitulo "Entidad encargada de ordenar el giro del reconocimiento" - servicio "ObtenerRespuestaEjecutorSGR - EntidadOrdenaGiro" - Error:', error);
                });
        }
        ...
        function buscar() {
            ...
            var tipoEntidadId = document.getElementById("ddlTipoEntidadGiro").value;
            tipoEntidadId = tipoEntidadId.replace('number:', '');
            var entidadId = document.getElementById("ddlEntidadGiro").value;
        ...
        function eliminarEjecutor(entity) {
            utilidades.mensajeWarning(
                ...
                function funcionContinuar() {
                    ...
                    utilidades.mensajeSuccess("La entidad encargada de ordenar giro del reconocimiento se eliminó con éxito.", false, "Es necesario incluir una entidad en esta tabla.", false);
                },
                ...
                "Aceptar",
                "Cancelar",
                "Se eliminará la entidad encargada de ordenar el giro del reconocimiento."
            );
        };
        ...
        vm.changeEntidad = function () {
            var idTipoE = document.getElementById("ddlTipoEntidadGiro").value;
            ...
            designarEjecutorSgrServicio.ObtenerEjecutorByTipoEntidad(idTipoE).then(
                ...
                utilidades.mensajeError('Error en el capitulo "Entidad encargada de ordenar el giro del reconocimiento" - servicio "ObtenerEjecutorByTipoEntidad".');
                console.error('Error en el capitulo "Entidad encargada de ordenar el giro del reconocimiento" - servicio "ObtenerEjecutorByTipoEntidad" - Error:', error);
            });
        };
        ...
        vm.guardar = function () {
            if (vm.aplicaCostos === null || vm.aplicaCostos === undefined || vm.aplicaCostos === "") {
                utilidades.mensajeError('Es necesario responder la pregunta "El proyecto aplica para el reconocimiento de costos de la estructuración".', false);
                return false;
            }

            if (vm.aplicaCostos === true && vm.tieneEjecutor === false) {
                utilidades.mensajeError('Es necesario diligenciar la entidad encargada de ordenar el giro del reconocimiento.', false);
                return false;
            }

            if (vm.aplicaCostos === false) {
                guardarCostos('AplicaReconCostos').then(function (ok1) {
                    if (!ok1)
                        return handleGuardarError();

                    guardarEntidad('EntidadOrdenaGiro', 'NULL').then(function (ok2) {
                        if (ok2) {
                            designarEjecutorSgrServicio.notificarCambio({ recCostos: false });
                            designarEjecutorSgrServicio.notificarCambio({ aplicaCostos: false });
                ...
                guardarCostos('AplicaReconCostos').then(function (ok1) {
                    ...
                    guardarEntidad('EntidadOrdenaGiro', vm.idEjecutor).then(function (ok2) {
                        if (ok2) {
                            designarEjecutorSgrServicio.notificarCambio({ recCostos: true });
                            return handleGuardarExito();
                        } else
        ...
        function guardarCostos(campo) {
            var data = {
                campo: campo,
                respuesta: !!vm.aplicaCostos,
        ...
        function guardarEntidad(campo, valor) {
            var data = {
                campo: campo,
                respuesta: valor,
                proyectoId: vm.proyectoId
        ...
        function handleGuardarError() {
            utilidades.mensajeError('Error al guardar capítulo "Entidad encargada de ordenar el giro del reconocimiento".', false);
        };
    ...
    angular.module('backbone').component('giroReconocimientoSgr', {
        templateUrl: "/src/app/formulario/ventanas/SGR/gestionRecursos/ejecutor/costosEstructuracion/giroReconocimientoSgr/giroReconocimientoSgr.html",
        controller: giroReconocimientoSgrController,
        controllerAs: "vm",
        bindings: {
            guardadoevent: '&',
            notificacionestado: '&',
