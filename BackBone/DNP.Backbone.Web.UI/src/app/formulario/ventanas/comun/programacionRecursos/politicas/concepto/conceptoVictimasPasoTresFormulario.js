var conceptoVictimasPasoTresCtrl = null;

(function () {
    'use strict';
    angular.module('backbone').controller('conceptoVictimasPasoTres', conceptoVictimasPasoTres);
    conceptoVictimasPasoTres.$inject = [
        '$scope',
        '$sessionStorage',
        '$timeout',
    ...
    function conceptoVictimasPasoTres(
        $scope,
        $sessionStorage,
        $timeout,
        focalizacionAjustesServicio,
        ...
        utilidades) {
        var vm = this;
        conceptoVictimasPasoTresCtrl = vm;
        //variables
        vm.lang = "es";
        vm.TabActivo = 1;
        ...
        vm.TienePoliticas = 0;
        vm.nombreComponente = "conceptoVictimasPasoTresFormulario";
        vm.activar = false;
        ...
        vm.cargarInicio = function () {
            setTimeout(function () {
                ...
                vm.ObtenerPoliticasSolicitudConcepto(vm.codigobpin);
        ...
        vm.activarControles = function (evento) {
            switch (evento) {
                case "inicio":
                    ...
                    document.getElementById('fileArchivoConcepto_' + vm.proyectoid).value = "";
        ...
        function adjuntarArchivo() {
            ...
            document.getElementById('fileArchivoConcepto_' + vm.proyectoid).value = "";
            document.getElementById('fileArchivoConcepto_' + vm.proyectoid).click();
        ...
        vm.ObtenerResumenSolicitudConcepto = function (bpin) {
            vm.activar = false;
            return focalizacionAjustesServicio.ObtenerResumenSolicitudConcepto(bpin, usuarioDNP, $sessionStorage.idNivel).then(
                function (respuesta) {
                    if (respuesta.data != null && respuesta.data != "") {
                        vm.ResumenSolicitudConcepto = [];
                        var arreglolistas = jQuery.parseJSON(respuesta.data);
                        ...
                        vm.ResumenSolicitudConcepto.forEach(rs => {
                            if (rs.Resumen) {
                                vm.ResumenSolicitudDetalle = rs.Resumen.filter(x => x.Aprobacion == "Pendiente");
                                if (vm.ResumenSolicitudDetalle.length <= 0) {
                                    vm.ResumenSolicitudDetalle = [];
                                    let result = rs.Resumen.reduce((a, b) => a.Fecha > b.Fecha ? a : b);
                                    if (result) {
        ...
        vm.GuardarConceptoVictimas = function () {
            let politicaId = 20; //Política de víctimas
    ...
    angular.module('backbone')
        .component('conceptoVictimasPasoTres', {
            templateUrl: 'src/app/formulario/ventanas/comun/programacionRecursos/politicas/concepto/conceptoVictimasPasoTresFormulario.html',
            controller: 'conceptoVictimasPasoTres',
            controllerAs: 'vm',
            bindings: {
                callback: '&',
