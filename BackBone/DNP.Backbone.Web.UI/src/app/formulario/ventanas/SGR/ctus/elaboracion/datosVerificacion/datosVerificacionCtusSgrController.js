(function () {
    ...
    datosVerificacionCtusSgrController.$inject = ['$scope', 'utilidades', '$sessionStorage'];

    function datosVerificacionCtusSgrController(
        $scope,
        utilidades,
        $sessionStorage
    ) {
        var vm = this;
        vm.nombreComponente = "sgrctuselaboraciondatosverificacion";
        vm.nombreComponenteHijo = "sgrctuselaboraciondatosverificaciondatosadicionales";
        ...
        vm.handlerComponentes = [
            { id: 1, componente: 'sgrctuselaboraciondatosverificaciondatosadicionales', handlerValidacion: null, handlerCambios: null, esValido: true },
            { id: 2, componente: 'sgrctuselaboraciondatosverificacionagregarsectores', handlerValidacion: null, handlerCambios: null, esValido: true }
        ...
        vm.guardado = function (nombreComponenteHijo) {
            vm.callback();
            if (nombreComponenteHijo == "sgrctuselaboraciondatosverificaciondatosadicionales") {
                vm.guardadocomponent({ nombreComponente: vm.nombreComponente, nombreComponenteHijo: nombreComponenteHijo });
                vm.notificarrefrescoSector();
            }

            if (nombreComponenteHijo == "sgrctuselaboraciondatosverificacionagregarsectores") {
                vm.guardadocomponent({ nombreComponente: vm.nombreComponente, nombreComponenteHijo: nombreComponenteHijo });
            }
        }
        ...
        vm.notificarRefresco = function (handler, nombreComponente) {
            if (nombreComponente == "sgrctuselaboraciondatosverificacionagregarsectores" ) {
        ...
        vm.inicializarComponenteCheck = function () {
            vm.handlerComponentesChecked = {
                'sgrctuselaboraciondatosverificaciondatosadicionales': true,
                'sgrctuselaboraciondatosverificacionagregarsectores': true
            };
        }
    ...
    angular.module('backbone').component('datosVerificacionCtusSgr', {
        templateUrl: "/src/app/formulario/ventanas/SGR/ctus/elaboracion/datosVerificacion/datosVerificacionCtusSgr.html",

        controller: datosVerificacionCtusSgrController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
