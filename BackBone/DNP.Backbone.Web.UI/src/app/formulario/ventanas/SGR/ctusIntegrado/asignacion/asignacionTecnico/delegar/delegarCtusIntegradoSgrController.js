(function () {
    'use strict';
    delegarCtusIntegradoSgrController.$inject = [
        '$scope',
        '$location',
        'utilidades',
        '$sessionStorage',
    ...
    function delegarCtusIntegradoSgrController(
        $scope,
        $location,
        utilidades,
        $sessionStorage,
        ...
        vm.user = {};
        vm.lang = "es";
        vm.nombreComponente = 'sgrctusintegradoasignaciondelegar';
        ...
        vm.init = function () {
            ValidarRolCtus(vm.proyectoId, vm.idInstancia);
        ...
        vm.delegarConceptoENChange = function () {
            if (vm.asignacionTecnico.delegaViabilidad == false) {
                ...
                ctusIntegradoAsignacionSgrServicio.notificarCambio({ reqEntidad: true });
            }
        }
        ...
        function ValidarEntidadAdscrita(rolDirectorId) {
            ctusIntegradoAsignacionSgrServicio.SGR_Proyectos_ValidarEntidadDelegada(vm.proyectoId, 'ctus')
                .then(function (response) {
                    if (response.data.Exito) {
                        vm.tipoVisualizacionDelegar = 'tipo1';
                        ctusIntegradoAsignacionSgrServicio.notificarCambio({ tipoVisualizacion: 'tipo2' });
        ...
        function ObtenerEntidadesAdscritas(proyectoId) {
            return ctusIntegradoAsignacionSgrServicio.SGR_Proyectos_LeerEntidadesAdscritas(proyectoId, 'integrado')
                .then(function (response) {
        ...
        vm.ActivarEditar = function () {
            if (vm.activar == true) {
                ...
                utilidades.mensajeWarning("¿Está seguro de continuar?", function funcionContinuar() {
                    ...
                    ctusIntegradoAsignacionSgrServicio.notificarCambio({ tipoVisualizacion: 'tipo1' });
                    OkCancelar();
                    vm.estadoBotonEdit = 'EDITAR';
                    vm.activar = true;
        ...
        function Guardar(proyectoId, entityId) {
            return ctusIntegradoAsignacionSgrServicio.SGR_Proyectos_ActualizarEntidadAdscritaCTUS(proyectoId, entityId, 'ctusIntegrado')
                .then(function (response) {
                    if (response) {
                        ctusIntegradoAsignacionSgrServicio.notificarCambio({ actualizarCapitulo: 'true' });
                        guardarCapituloModificado();
                        vm.estadoBotonEdit = 'EDITAR';
                        vm.activar = true;
                        utilidades.mensajeSuccessRedirect(`Se ha delegado la elaboración del concepto integrado a la entidad ${vm.nombreEntidadSeleccionada}.`,
    ...
    angular.module('backbone').component('delegarCtusIntegradoSgr', {
        templateUrl: "/src/app/formulario/ventanas/SGR/ctusIntegrado/asignacion/asignacionTecnico/delegar/delegarCtusIntegradoSgr.html",
        controller: delegarCtusIntegradoSgrController,
        controllerAs: "vm",
        bindings: {
            guardadoevent: '&',
            notificacionestado: '&',
