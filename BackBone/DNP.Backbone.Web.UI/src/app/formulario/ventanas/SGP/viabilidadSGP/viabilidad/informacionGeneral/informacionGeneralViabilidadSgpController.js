(function () {
    ...
    informacionGeneralViabilidadSgpController.$inject = [
        '$scope',
        '$sessionStorage',
    ];

    function informacionGeneralViabilidadSgpController(
        $scope,
        $sessionStorage,
    ) {
        var vm = this;
        ...
        vm.nombreComponente = "sgpviabilidadviabilidadInformacionBasica";
        vm.tipotramiteid = $sessionStorage.tipotramiteid;
        vm.tramiteid = $sessionStorage.tipotramiteid;
        vm.nombreComponenteHijo = "sgpviabilidadviabilidadInformacionBasicausuariosinvolucradossgp";
        ...
        vm.handlerComponentes = [
            { id: 1, componente: 'sgpviabilidadviabilidadInformacionBasicausuariosinvolucradossgp', handlerValidacion: null, handlerCambios: null, esValido: true },
            { id: 2, componente: 'sgpviabilidadviabilidadInformacionBasicaviabilidadsectorialsgp', handlerValidacion: null, handlerCambios: null, esValido: true }
        ];
       ...
       * */
        ...
        vm.inicializarComponenteCheck = function () {
            vm.handlerComponentesChecked = {
                'sgpviabilidadviabilidadInformacionBasicausuariosinvolucradossgp': true,
                'sgpviabilidadviabilidadInformacionBasicaviabilidadsectorialsgp': true
            };
        }
        ...
        */
        vm.notificacionValidacionEvent = function (listErrores) {
            ...
            if (erroresList.length > 0) {
                ...
                const alertError = "alert-" + vm.nombreComponente;
                let spansAlertError = [];

                for (var i = 0; i < spans.length; i++) {
                    let span = spans[i];
                    let id = span.getAttribute("id");
                    if (id && id.indexOf(alertError) !== -1) {
                        spansAlertError.push(span);
                    }
                }
                spansAlertError.forEach(function (item) {
                    item.classList.remove("ico-advertencia");
                });

                var campomensajeerror = document.getElementById("SGRERRSEC" + vm.nombreComponente);
    ...
    angular.module('backbone').component('informacionGeneralViabilidadSgp', {
        templateUrl: "/src/app/formulario/ventanas/SGP/viabilidadSGP/viabilidad/informacionGeneral/informacionGeneralViabilidadSgp.html",
        controller: informacionGeneralViabilidadSgpController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
