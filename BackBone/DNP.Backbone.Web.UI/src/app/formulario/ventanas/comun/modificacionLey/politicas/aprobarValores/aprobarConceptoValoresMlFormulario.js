(function () {
    ...
    aprobarConceptoValoresMl.$inject = ['$scope', '$sessionStorage', '$uibModal', 'utilidades',
    ...
    function aprobarConceptoValoresMl(
        $scope,
        $sessionStorage,
        $uibModal,
        utilidades,
        ...
        var vm = this;
        vm.init = init;
        vm.nombreComponente = "aprobarConceptoValoresMlFormulario";
        ...
        $scope.$watch('vm.conceptopoliticavalores', function () {
        ...
        vm.ActivarEditar = function (politicaId, resumen) {
            ...
            var variable = $("#Editar" + vm.conceptopoliticavalores)[0].innerText;

            var radiosi = document.getElementById("radio" + vm.conceptopoliticavalores + resumen.Preguntas[0].PreguntaId + 0);
            var radiono = document.getElementById("radio" + vm.conceptopoliticavalores + resumen.Preguntas[0].PreguntaId + 1);
            var obs = document.getElementById("obs" + vm.conceptopoliticavalores + resumen.Preguntas[0].PreguntaId);

            if (variable == "EDITAR") {
                $("#Editar" + vm.conceptopoliticavalores).html("CANCELAR");

                document.getElementById("Guardarpsca" + vm.conceptopoliticavalores).classList.remove('btnguardarDisabledDNP');
                document.getElementById("Guardarpsca" + vm.conceptopoliticavalores).classList.add('btnguardarDNP');
                document.getElementById("Enviarpsca" + vm.conceptopoliticavalores).classList.remove('btnguardarDNP');
                document.getElementById("Enviarpsca" + vm.conceptopoliticavalores).classList.add('btnguardarDisabledDNP');
                ...
                utilidades.mensajeWarning("¿Está seguro de continuar?", function funcionContinuar() {
                    ...
                    document.getElementById("Guardarpsca" + vm.conceptopoliticavalores).classList.add('btnguardarDisabledDNP');
                    document.getElementById("Guardarpsca" + vm.conceptopoliticavalores).classList.remove('btnguardarDNP');
                    ...
                    if (radiosi != undefined) {
                        $('#radio' + vm.conceptopoliticavalores + resumen.Preguntas[0].PreguntaId + 0).attr("disabled", true);
                        $('#radio' + vm.conceptopoliticavalores + resumen.Preguntas[0].PreguntaId + 1).prop('checked', false);
                    }

                    if (radiono != undefined) {
                        $('#radio' + vm.conceptopoliticavalores + resumen.Preguntas[0].PreguntaId + 1).attr("disabled", true);
                        $('#radio' + vm.conceptopoliticavalores + resumen.Preguntas[0].PreguntaId + 1).prop('checked', false);
                    }

                    if (obs != undefined) {
                        obs.value = observacionpregunta;
                        $('input[type =obs' + vm.conceptopoliticavalores + resumen.Preguntas[0].PreguntaId + '], textarea').attr('readonly', 'readonly');
                    }

                    $("#Editar" + vm.conceptopoliticavalores).html("EDITAR");
                    $("#obserrorconceptoajuste" + vm.conceptopoliticavalores).attr('disabled', true);
                    $("#obserrorconceptoajuste" + vm.conceptopoliticavalores).fadeOut();
                    $("#obserrorconceptoajustemsn" + vm.conceptopoliticavalores).attr('disabled', true);
                    $("#obserrorconceptoajustemsn" + vm.conceptopoliticavalores).fadeOut();
                    var Errormsn = document.getElementById("obserrorconceptoajustemsn" + vm.conceptopoliticavalores);
                    if (Errormsn != undefined) {
                        Errormsn.innerHTML = '<span></span>';
                    }

                    $("#radioerrorconceptoajuste" + vm.conceptopoliticavalores).attr('disabled', true);
                    $("#radioerrorconceptoajuste" + vm.conceptopoliticavalores).fadeOut();
                    $("#radioerrorconceptoajustemsn" + vm.conceptopoliticavalores).attr('disabled', true);
                    $("#radioerrorconceptoajustemsn" + vm.conceptopoliticavalores).fadeOut();
                    var Errormsn = document.getElementById("radioerrorconceptoajustemsn" + vm.conceptopoliticavalores);
                    if (Errormsn != undefined) {
                        Errormsn.innerHTML = '<span></span>';
                    }
                ...
                focalizacionAjustesServicio.GuardarPreguntasEnvioPoliticaSubDireccionAjustes(RespuestaPoliticasSubdireccion).then(function (response) {
                    if (response.statusText === "OK" || response.status === 200) {
                        ...
                        if (i == vm.EnviosSubdireccion.length) {
                            $("#Editar" + vm.conceptopoliticavalores).html("EDITAR");
                            document.getElementById("Enviarpsca" + vm.conceptopoliticavalores).classList.remove('btnguardarDisabledDNP');
                            document.getElementById("Enviarpsca" + vm.conceptopoliticavalores).classList.add('btnguardarDNP');
                            document.getElementById("Guardarpsca" + vm.conceptopoliticavalores).classList.remove('btnguardarDNP');
                            document.getElementById("Guardarpsca" + vm.conceptopoliticavalores).classList.add('btnguardarDisabledDNP');
                            $('#radio' + vm.conceptopoliticavalores + resumen.Preguntas[0].PreguntaId + 0).attr("disabled", true);
                            $('#radio' + vm.conceptopoliticavalores + resumen.Preguntas[0].PreguntaId + 1).attr("disabled", true);
        ...
        function validarCampos(politicaId, resumen) {
            ...
            if (resumen.Preguntas[0].ObservacionPregunta == "" || resumen.Preguntas[0].ObservacionPregunta == null) {
                $("#obserrorconceptoajuste" + vm.conceptopoliticavalores).attr('disabled', false);
                $("#obserrorconceptoajuste" + vm.conceptopoliticavalores).fadeIn();
                $("#obserrorconceptoajustemsn" + vm.conceptopoliticavalores).attr('disabled', false);
                $("#obserrorconceptoajustemsn" + vm.conceptopoliticavalores).fadeIn();
                var Errormsn = document.getElementById("obserrorconceptoajustemsn" + vm.conceptopoliticavalores);
                if (Errormsn != undefined) {
            ...
            if (resumen.Preguntas[0].Respuesta == "" || resumen.Preguntas[0].Respuesta == null) {
                $("#radioerrorconceptoajuste" + vm.conceptopoliticavalores).attr('disabled', false);
                $("#radioerrorconceptoajuste" + vm.conceptopoliticavalores).fadeIn();
                $("#radioerrorconceptoajustemsn" + vm.conceptopoliticavalores).attr('disabled', false);
                $("#radioerrorconceptoajustemsn" + vm.conceptopoliticavalores).fadeIn();
                var Errormsn = document.getElementById("radioerrorconceptoajustemsn" + vm.conceptopoliticavalores);
                if (Errormsn != undefined) {
                    Errormsn.innerHTML = '<span style="margin: 0 auto;">Campo Obligatorio</span>';
                }
        ...
        vm.ConsultarArchivosConcepto = function () {
            ...
            let param = {
                ...
                politicaId: Number(vm.conceptoPoliticaValores),
                status: "Nuevo",
    ...
    angular.module('backbone').component('aprobarConceptoValoresMl', {
        templateUrl: "src/app/formulario/ventanas/comun/modificacionLey/politicas/aprobarValores/aprobarConceptoValoresMlFormulario.html",
        controller: aprobarConceptoValoresMl,
        controllerAs: "vm",
        bindings: {
            callback: '&',
