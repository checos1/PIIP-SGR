(function () {
    ...
    recursosSgrController.$inject = ['$scope', 'utilidades', '$sessionStorage'];

    function recursosSgrController(
        $scope,
        utilidades,
        $sessionStorage,
    ) {
        var vm = this;
        vm.nombreComponente = "sgrviabilidadpreviosrecursos";
        vm.nombreComponenteHijo = "sgrviabilidadpreviosrecursosfuentessgr";
        ...
        vm.handlerComponentes = [
            { id: 1, componente: 'sgrviabilidadpreviosrecursosfuentessgr', handlerValidacion: null, handlerCambios: null, esValido: true },
            { id: 2, componente: 'sgrviabilidadpreviosrecursosfuentesnosgr', handlerValidacion: null, handlerCambios: null, esValido: true, errores: null },
            { id: 3, componente: 'sgrviabilidadpreviosrecursosresumencostos', handlerValidacion: null, handlerCambios: null, esValido: true, errores: null },
       ...
       * */
        ...
        vm.inicializarComponenteCheck = function () {
            vm.handlerComponentesChecked = {
                'sgrviabilidadpreviosrecursosfuentessgr': true,
                'sgrviabilidadpreviosrecursosfuentesnosgr': true,
                'sgrviabilidadpreviosrecursosresumencostos': true,
            };
        }
        ...
        */
        vm.notificacionValidacionEvent = function (listErrores) {
            ...
            if (erroresList.length > 0) {
                ...
                vm.erroresComponente = [];
                //var errorElements = document.getElementsByClassName('errorpregDatosPre');
                //var testDivs = Array.prototype.filter.call(errorElements, function (errorElement) {
                //    errorElement.innerHTML = "";
                //    errorElement.classList.add('hidden');
                //});

                //errorElements = document.getElementsByClassName('errorSeccionPrevios');
        ...
        */
        ...
        vm.limpiarErrores = function () {
            ...
            var elementos = document.querySelectorAll('[id^="CRCVSS"]');
            elementos.forEach(function (elemento) {
                elemento.classList.add('hidden');
            });

            vm.handlerComponentes.forEach(element => {
                var idSpanAlertComponent = document.getElementById("alert-" + element.componente);
                if (idSpanAlertComponent != undefined) {
                    idSpanAlertComponent.classList.remove("ico-advertencia");
                }
            })
    ...
    angular.module('backbone').component('recursosSgr', {
        templateUrl: "/src/app/formulario/ventanas/SGR/viabilidadSGR/previos/recursos/recursosSgr.html",

        controller: recursosSgrController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
