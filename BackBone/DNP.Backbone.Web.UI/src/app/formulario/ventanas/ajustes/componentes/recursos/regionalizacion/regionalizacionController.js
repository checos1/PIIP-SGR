(function () {
    ...
    regionalizacionController.$inject = ['$scope', 'recursosAjustesServicio', '$sessionStorage', '$uibModal', 'utilidades',
        'constantesBackbone', '$timeout', 'justificacionCambiosServicio'
    ];

    function regionalizacionController(
        $scope,
        ...
        vm.obtenerRegionalizacion = function (bpin) {
            ...
            vm.ProductoRegionalizadoActual = [];
            vm.modelo = null;
            return recursosAjustesServicio.obtenerRegionalizacionGeneral($sessionStorage.idInstancia).then(
                function (respuesta) {
        ...
        function actualizaFila(event, cantidad, vigencias, suma) {
            ...
            if (suma == 1) {
                ...
                angular.forEach(vigencias, function (series) {
                    acumula = acumula + parseFloat(series.EnAjuste);
        ...
        vm.validateFormat = function (event, cantidad) {
            ...
            if (permitido) {
                ...
                if (n[1]) {
                    ...
                    if (spiltArray.length === 0) return;
                    if (spiltArray.length === 1 && (spiltArray[0] == '-' || spiltArray[0] === '.')) return;
                    if (spiltArray.length === cantidad && newValue === '-.') return;
                    ...
                    if ((n[1].length == cantidad && n[1] > 99) || (n[1].length > cantidad && n[1] > 99)) {
                        event.preventDefault();
                    }
                }
            } else {
                        ...
                        producto.Localizaciones.forEach(localizacion => {
                            ...
                            localizacion.Vigencias.forEach(vigencia => {
                                data.push({
                                    ...
                                    TotalRegionalizadoFuente: fuente.TotalRegionalizadoFuente,
                                    Producto: producto.Producto,
                                    TotalCostoProducto: producto.TotalCostoProducto,
                                    TotalRegionalizacionProducto: producto.TotalRegionalizacionProducto,
                                    Localizacion: localiza,
                                    Vigencia: vigencia.Vigencia,
                                    EnAjuste: parseFloat(vigencia.EnAjuste).toFixed(2), //ConvertirNumero2decimales(vigencia.EnAjuste),// parseFloat(vigencia.EnAjuste),//.replaceAll('.', ','),
                                    MetaEnAjuste: parseFloat(vigencia.MetaEnAjuste).toFixed(4), //ConvertirNumero4decimales(vigencia.MetaEnAjuste)  //parseFloat(vigencia.MetaEnAjuste),//.replaceAll('.', ','),
                                });
                            });
                        });
                    });
                            ...
                            reader.onload = function (e) {
                                ...
                                resultado = excelRows.map(function (item, index) {
                                    ...
                                    */
                                    ...
                                    else if (!ValidarDicimal(item["MetaEnAjuste"].toString(), 4)) {
        ...
        function GuardarArchivo(valorescero, cargamasiva) {
            if (cargamasiva)
                vm.activarControles('inicio');
            return recursosAjustesServicio.guardarRegionalizacionFuentes(vm.FuenteArchivo, vm.idUsuario).then(function (response) {
        ...
        vm.validarAFFR003 = function (errores) {
            ...
            for (var i = 0; i < vm.modelo.Fuentes.length; i += 1) {
                ...
                for (var j = 0; j < vm.modelo.Fuentes[i].Productos.length; j += 1) {
                    ...
                    for (var k = 0; k < vm.modelo.Fuentes[i].Productos[j].Localizaciones.length; k += 1) {
                        for (var m = 0; m < vm.modelo.Fuentes[i].Productos[j].Localizaciones[k].Vigencias.length; m += 1) {
                            if (vm.modelo.Fuentes[i].Productos[j].Localizaciones[k].Vigencias[m].EnAjuste == 0 && vm.modelo.Fuentes[i].Productos[j].Localizaciones[k].Vigencias[m].MetaEnAjuste > 0) {
                                var localizacionIconError = document.getElementById("localizacionIconError-" + vm.modelo.Fuentes[i].FuenteId + "-" + vm.modelo.Fuentes[i].Productos[j].ProductoId + "-" + vm.modelo.Fuentes[i].Productos[j].Localizaciones[k].LocalizacionId + "-" + vm.modelo.Fuentes[i].Productos[j].Localizaciones[k].Vigencias[m].Vigencia);
                                if (localizacionIconError != undefined) {
                                    localizacionIconError.classList.remove('hidden');
                                }
                                var localizacionError = document.getElementById("localizacionError-" + vm.modelo.Fuentes[i].FuenteId + "-" + vm.modelo.Fuentes[i].Productos[j].ProductoId + "-" + vm.modelo.Fuentes[i].Productos[j].Localizaciones[k].LocalizacionId);
                                if (localizacionError != undefined) {
                                    localizacionError.classList.remove('hidden');
                                }
                                var localizacionFooterError = document.getElementById("localizacionFooterError-" + vm.modelo.Fuentes[i].FuenteId + "-" + vm.modelo.Fuentes[i].Productos[j].ProductoId + "-" + vm.modelo.Fuentes[i].Productos[j].Localizaciones[k].LocalizacionId);
                                if (localizacionFooterError != undefined) {
                                    localizacionFooterError.classList.remove('hidden');
                                }
                                var RegionalizacionLocalizacionIconError = document.getElementById("RegionalizacionLocalizacionIconError-" + vm.modelo.Fuentes[i].FuenteId + "-" + vm.modelo.Fuentes[i].Productos[j].ProductoId);
                                if (RegionalizacionLocalizacionIconError != undefined) {
                                    RegionalizacionLocalizacionIconError.classList.remove('hidden');
                                }
                                var RegionalizacionFuenteIconError = document.getElementById("RegionalizacionFuenteIconError-" + vm.modelo.Fuentes[i].FuenteId);
                                if (RegionalizacionFuenteIconError != undefined) {
                                    RegionalizacionFuenteIconError.classList.remove('hidden');
                                }
                            }
                        }
        ...
        vm.ErrorFuente = function (errores) {
            var FuenteId = errores.split('|')[0];
            ...
            if (ErrorFuente != undefined) {
                if (ErrorFuentemsn != undefined) {
                    ErrorFuentemsn.innerHTML = '<span>' + MensajeError + "</span>";
                    ErrorFuente.classList.remove('hidden');
                }
            }
        }

        vm.ErrorProducto = function (errores) {
            /*
                 var ProductoId = errores.split('|')[0];
                 var MensajeError = errores.split('|')[1];;
 
                 var ErrorProducto = document.getElementById("ErrorProducto-" + ProductoId);
                 var ErrorProductomsn = document.getElementById("ErrorProductoErrorMsg-" + ProductoId);
 
                 if (ErrorProducto != undefined) {
                     if (ErrorProductomsn != undefined) {
                         ErrorProductomsn.innerHTML = '<span>' + MensajeError + "</span>";
    ...
    angular.module('backbone').component('regionalizacion', {
        templateUrl: "src/app/formulario/ventanas/ajustes/componentes/recursos/regionalizacion/regionalizacion.html",
        controller: regionalizacionController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
