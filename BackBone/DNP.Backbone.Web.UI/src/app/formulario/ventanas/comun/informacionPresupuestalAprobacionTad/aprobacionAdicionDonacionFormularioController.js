(function () {
    ...
    aprobacionAdicionDonacionFormularioController.$inject = [
        '$sessionStorage',
        '$scope',
        'utilidades',
    ...
    function aprobacionAdicionDonacionFormularioController(
        $sessionStorage,
        $scope,
        utilidades,
        justificacionCambiosServicio,
        ...
        vm.nombreComponente = "informacionpresupuestalsolicitudadiciondonacion";
        ...
        $scope.$watch('vm.tramiteid', function () {
            if (vm.tramiteid !== '' && vm.tramiteid !== undefined && vm.tramiteid !== null) {
                if ($sessionStorage.tramiteId)
                    ObtenerAprobacionProyectosAsociadosTad();
            }
        });
        ...
        function ObtenerAprobacionProyectosAsociadosTad() {
            return comunesServicio.ObtenerPresupuestalProyectosAsociados_Adicion(vm.tramiteid, vm.instanciaId).then(
                function (respuesta) {
                    var arreglolistas = jQuery.parseJSON(respuesta.data);
                    //debugger;
                    console.log(jQuery.parseJSON(arreglolistas));
        ...
        vm.actualizaFila = function (event, asociados) {
            ...
            $scope.datos.ResumenProyectos[credito].Proyectos[0].TotalAprobadoNacion = TotalSolicitadoNacion;

            var TotalSolicitadoPropios = 0;
            angular.forEach(asociados.DetalleFuentes.filter(x => x.NombreTipoRecurso.includes('Propios')), function (series) {
                TotalSolicitadoPropios = TotalSolicitadoPropios + parseFloat(series.ValorIncorporarAprobadoCSF) + parseFloat(series.ValorIncorporarAprobadoSSF);
            });
            $scope.datos.ResumenProyectos[credito].Proyectos[0].TotalAprobadoPropios = TotalSolicitadoPropios;
        ...
        vm.actualizaFila2 = function (event, aportantes) {
            ...
            $scope.datos.ResumenProyectos[contrato].Proyectos[0].TotalAprobadoNacion = TotalSolicitadoNacion;

            var TotalSolicitadoPropios = 0;
            angular.forEach(aportantes.DetalleFuentes.filter(x => x.NombreTipoRecurso.includes('Propios')), function (series) {
                TotalSolicitadoPropios = TotalSolicitadoPropios + parseFloat(series.ValorIncorporarAprobadoCSF) + parseFloat(series.ValorIncorporarAprobadoSSF);
            });
            $scope.datos.ResumenProyectos[contrato].Proyectos[0].TotalAprobadoPropios = TotalSolicitadoPropios;
    ...
    angular.module('backbone').component('aprobacionAdicionDonacionFormulario', {
        templateUrl: "src/app/formulario/ventanas/comun/informacionPresupuestalAprobacionTad/aprobacionAdicionDonacionFormulario.html",
        controller: aprobacionAdicionDonacionFormularioController,
        controllerAs: "vm",
        bindings: {
            guardadoevent: '&',
            notificacioncambios: '&',
