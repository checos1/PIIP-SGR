(function () {
    ...
    aprobacionF1ApruebaOpcSgrController.$inject = ['$scope', 'utilidades', '$sessionStorage'];

    function aprobacionF1ApruebaOpcSgrController(
        $scope,
        utilidades,
        $sessionStorage,
    ) {
        var vm = this;
        vm.nombreComponente = "sgraprobacion1aprobacionopc";
        vm.nombreComponenteHijo = "sgraprobacion1aprobacionopcregistroaprobacionopc";
        ...
        vm.ProyectoTieneInstanciasNoOPC = $sessionStorage.ProyectoTieneInstanciasNoOPC;

        if (vm.ProyectoTieneInstanciasNoOPC) {
            vm.handlerComponentes = [
                { id: 1, componente: 'sgraprobacion1aprobacionopcregistroaprobacionopc', handlerValidacion: null, handlerCambios: null, esValido: true },
                { id: 2, componente: 'sgraprobacion1aprobacionopcresumenaprobacionopc', handlerValidacion: null, handlerCambios: null, esValido: true },
                { id: 3, componente: 'sgraprobacion1aprobacionopcresumenaprobacion', handlerValidacion: null, handlerCambios: null, esValido: true },
            ];
        } else {
            vm.handlerComponentes = [
                { id: 1, componente: 'sgraprobacion1aprobacionopcregistroaprobacionopc', handlerValidacion: null, handlerCambios: null, esValido: true },
        ...
        */
        ...
        vm.capitulos = function (listadoCapitulos) {
            ...
            listadoCapRecursos.forEach(function (item) {
                ...
                if (item.nombreComponente == 'sgraprobacion1aprobacionopcregistroaprobacionopc' && vm.TieneOPC && vm.PorcentajeCredito < 100) {
                    item.Capitulo = 'Registro decisión de aprobación'
                }
                if (item.nombreComponente == 'sgraprobacion1aprobacionopcresumenaprobacion' && vm.ProyectoTieneInstanciasNoOPC) { 
    ...
    angular.module('backbone').component('aprobacionF1ApruebaOpcSgr', {
        templateUrl: "/src/app/formulario/ventanas/SGR/gestionRecursos/aprobacion/F1/aprobacion/aprobacionF1ApruebaOpcSgr.html",
        controller: aprobacionF1ApruebaOpcSgrController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
