(function () {
	...
	ResumenFocalizacionSgpController.$inject = ['$scope', '$sessionStorage', '$uibModal', 'utilidades', 'focalizacionAjustesSGPServicio', 'justificacionCambiosServicio', 'transversalSgpServicio'];

	function ResumenFocalizacionSgpController(
		$scope,
		$sessionStorage,
		$uibModal,
		utilidades,
		...
		vm.componentesRefresh = [
			...
			'sgpsolicitudrecursosfocalizacioncrucepoliticassgp',
			'sgprecursosfuentesfinanciacionsgp',
			'sgpsolicitudrecursosfocalizacionresumenfocalizacionsgp'
		];
		...
		function abrirModalAgregarIndicador(CategoiaId1, CategoriaId2, Subcategoria, Categoria, Politica, FocalizacionId) {
			...
			$uibModal.open({
				templateUrl: 'src/app/formulario/ventanas/SGP/gestionRecursosSGP/componentes/focalizacion/ResumenFocalizacion/ResumenFocalizacionCategoria/Modal/modalAgregarIndicadorSgp.html',
				controller: 'modalAgregarIndicadorSgpController',
				controllerAs: "vm",
				size: 'lg',
		...
		function abrirModalAgregarCategoriaPolitica(Politica) {
			...
			let modalInstance = $uibModal.open({
				...
				templateUrl: "src/app/formulario/ventanas/SGP/gestionRecursosSGP/componentes/focalizacion/ResumenFocalizacion/FocalizacionCategoria/modalAgregarCategoriaPoliticaSgp.html",
				controller: 'modalAgregarCategoriaPoliticaSgpController',
				controllerAs: "vm",
		...
		function _validarTotalBeneficiarios(producto, focalizacionAjustada) {
			...
			if (producto.EsAcumulativo === "NO") {
				for (let foc of focalizacionAjustada) {
					if (foc.PersonasCategoria > Number(producto.PoblacionBeneficiaria.replace(/\./g, '').replace(',', '.')))
					{
						validacion = false;
						break;
					}
				...
				focalizacionAjustada.some(foc => {
					const valor = Number(foc.PersonasCategoria);
					if (!isNaN(valor)) {
	...
	angular.module('backbone').component('resumenFocalizacionSgp', {
		templateUrl: "src/app/formulario/ventanas/SGP/gestionRecursosSGP/componentes/focalizacion/ResumenFocalizacion/ResumenFocalizacionSgp.html",
		controller: ResumenFocalizacionSgpController,
		controllerAs: "vm",
		bindings: {
			callback: '&',
