namespace DNP.ServiciosTransaccional.Web.API.Controllers.Proyecto
{
    ...
    public class BpinController : ApiController
    {
        private readonly IAutorizacionUtilidades _autorizacionUtilidades;
        private readonly IBpinServicio _bpinServicio;

        public BpinController(IBpinServicio bpinServicio, IAutorizacionUtilidades autorizacionUtilidades)
        {
            _bpinServicio = bpinServicio;
            _autorizacionUtilidades = autorizacionUtilidades;
        }

        [Route("api/Proyecto/DNP_SN_Pla_Viabilidad_GenerarBPINDto")]
        [HttpPost]
        public async Task<IHttpActionResult> GenerarBPIN(ObjetoNegocio contenido)
        {
            try
            {
                var respuestaAutorizacion = await _autorizacionUtilidades.ValidarUsuario(RequestContext.Principal.Identity.Name,
                                                           ...
                                                           ConfigurationManager.AppSettings["generarBpinProyecto"]);
                ...
                var response = await Task.Run(() => _bpinServicio.GenerarBPIN(parametrosActualizar, parametrosAuditoria));
        ...
        [Route("api/SGR/Proyecto/Viabilidad_GenerarBPIN")]
        [HttpPost]
        public async Task<IHttpActionResult> GenerarBPINSgr(ObjetoNegocio contenido)
        {
            try
            {
                var respuestaAutorizacion = await _autorizacionUtilidades.ValidarUsuario(RequestContext.Principal.Identity.Name,
                ...
                var response = await Task.Run(() => _bpinServicio.GenerarBPINSgr(parametrosActualizar, parametrosAuditoria));
                var respuesta = new HttpResponseMessage
                {
