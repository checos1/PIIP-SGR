(function () {
    ...
    documentoSoporteSgrController.$inject = [
        '$scope',
        '$sessionStorage',
    ];

    function documentoSoporteSgrController(
        $scope,
        $sessionStorage,
    ) {
        var vm = this;
        ...
        vm.nombreComponente = "sgrviabilidadpreviossoporte";
        vm.tipotramiteid = $sessionStorage.tipotramiteid;
        vm.tramiteid = $sessionStorage.tipotramiteid;
        vm.nombreComponenteHijo = "sgrviabilidadpreviossoportealojararchivo";
        ...
        var xRol = roles.filter(x => x.Nombre == "R_Formulador");
        if (xRol.length > 0) { vm.rol = xRol[0].IdRol;}
        

        vm.handlerComponentes = [
            { id: 1, componente: 'sgrviabilidadpreviossoportealojararchivo', handlerValidacion: null, handlerCambios: null, esValido: true }
        ];
        vm.handlerComponentesChecked = {};

        vm.init = function () {
            var accionPrevios = $sessionStorage.listadoAccionesTramite.find(f => f.Ventana == 'previosViabilidadSgr');
            if (accionPrevios != null && accionPrevios.Roles.length > 0) {
                vm.rol = accionPrevios.Roles[0].IdRol;
            }
       ...
       * */
        ...
        vm.inicializarComponenteCheck = function () {
            vm.handlerComponentesChecked = {
                'sgrviabilidadpreviossoportealojararchivo': true
            };
        }
        ...
        */
        vm.notificacionValidacionEvent = function (listErrores) {
            ...
            if (erroresList.length > 0) {
                ...
                var campomensajeerrorA = document.getElementById(vm.nombreComponenteHijo + "A-archivo-error");
                if (campomensajeerrorA != undefined) {
                    campomensajeerrorA.classList.add("hidden");
                }

                var campomensajeerrorB = document.getElementById(vm.nombreComponenteHijo + "B-archivo-error");
                if (campomensajeerrorB != undefined) {
                    campomensajeerrorB.classList.add("hidden");
                }

                var iconoAlertError = document.getElementById("alert-" + vm.nombreComponenteHijo);
                if (iconoAlertError != undefined) {
                    iconoAlertError.classList.remove("ico-advertencia");
                }
            }
        }
    ...
    angular.module('backbone').component('documentoSoporteSgr', {
        templateUrl: "src/app/formulario/ventanas/SGR/viabilidadSGR/previos/documentos/documentoSoporteSgr.html",
        controller: documentoSoporteSgrController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
