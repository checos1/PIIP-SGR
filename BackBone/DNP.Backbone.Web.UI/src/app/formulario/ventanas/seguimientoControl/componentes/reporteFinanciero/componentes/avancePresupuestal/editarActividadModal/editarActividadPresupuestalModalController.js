(function () {
    'use strict';
    angular.module('backbone').controller('editarActividadPresupuestalModalController', editarActividadPresupuestalModalController);

    editarActividadPresupuestalModalController.$inject = [
        '$uibModalInstance',
        '$sessionStorage',
        'utilidades',
        'Nivel',
        ...
        'UnidadesMedida',
        'editarActividadPresupuestalModalServicio'
    ];

    function editarActividadPresupuestalModalController(
        $uibModalInstance,
        $sessionStorage,
        utilidades,
        Nivel,
        ...
        function guardar() {
            if (isValido()) {
                ...
                editarActividadPresupuestalModalServicio.editarActividad(vm.editarActividad)
                    .then(resultado => {
        ...
        function validarPosposicion() {
            ...
            if (vm.editarActividad.ActividadPredecesora.Index != "" &&
                ...
                if (esValido && (vm.editarActividad.Tipo.Id == 'FC' ||
                    ...
                    var duracionPromedio = (parseInt(actPredecesora.DuracionOptimista) +
                        parseInt(actPredecesora.DuracionPesimista) +
                        parseInt(actPredecesora.DuracionProbable)) / 3;

                    esValido = ((vm.editarActividad.PosPosicion) <= (duracionPromedio * vm.configuracion.maximoPosposicion));
                    if (!esValido) {
                        vm.errores.PosPosicion = 'El Adelanto en la dependencia FC o CC no podrá ser superior al ' + (vm.configuracion.maximoPosposicion * 100) + '% de la duración promedio en días de la actividad Predecesora';
                    } else {
