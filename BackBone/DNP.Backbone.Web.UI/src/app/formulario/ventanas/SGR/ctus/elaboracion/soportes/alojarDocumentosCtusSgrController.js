(function () {
    ...
    alojarDocumentosCtusSgrController.$inject = [
        '$scope',
        '$sessionStorage',
    ];

    function alojarDocumentosCtusSgrController(
        $scope,
        $sessionStorage,
    ) {
        var vm = this;
        ...
        vm.nombreComponente = "sgrctussoportes";
        vm.tipotramiteid = $sessionStorage.tipotramiteid;
        vm.tramiteid = $sessionStorage.tipotramiteid;
        vm.nombreComponenteHijo = "sgrctussoportesalojararchivo";
        vm.notificacionCambiosCapitulos = null;
        vm.rol = null;

        vm.handlerComponentes = [
            { id: 1, componente: 'sgrctussoportesalojararchivo', handlerValidacion: null, handlerCambios: null, esValido: true }
        ];
        vm.handlerComponentesChecked = {};

        vm.init = function () {
            var accionCtus = $sessionStorage.listadoAccionesTramite.find(f => f.Ventana == 'ctusSgr');
            if (accionCtus != null && accionCtus.Roles.length > 0) {
                vm.rol = accionCtus.Roles[0].IdRol;
            }
    ...
    angular.module('backbone').component('alojarDocumentosCtusSgr', {
        templateUrl: "/src/app/formulario/ventanas/SGR/ctus/elaboracion/soportes/alojarDocumentosCtusSgr.html",
        controller: alojarDocumentosCtusSgrController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
