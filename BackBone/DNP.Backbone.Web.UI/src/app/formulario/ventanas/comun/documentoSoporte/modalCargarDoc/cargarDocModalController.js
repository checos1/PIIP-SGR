(function () {
    ...
    cargarDocModalController.$inject = [
        '$uibModalInstance',
        '$scope',
    ...
    function cargarDocModalController(
        $uibModalInstance,
        $scope,
        servicioFichasProyectos,
        $sessionStorage,
        ...
        vm.nombre = '';
        vm.initCargarArchivosModal = initCargarArchivosModal;
        ...
        function consultarTipoDocumento() {
            ...
            documentoSoporteServicios.ObtenerListaTipoDocumentosSoportePorRolTrv(tipoTramiteId, "A", idTramite, vm.IdNivel, vm.IdInstancia, vm.IdAccion)
                .then(function (resultado) {
                    ...
                    console.log("listaTipoArchivosModalNormal");
        ...
        function initCargarArchivosModal() {
        ...
        function onClickCargar() {
            ...
            let tipoArchivoUnico = vm.listaTipoArchivos.find(x => x.Id == vm.idTipoArchivoSeleccionado && x.Unico == true);
            ...
            //Validacion para no permitir cargar mas de un tipo de documento, hace falta verificar que tipos de documentos debe permitir
            let existeTipoDocumento = $sessionStorage.listaTipoArchivosFiltro.some(function (item) {
                if (vm.idTipoArchivoSeleccionado != 0)
                    return item.tipoDocumentoId === tipoArchivoUnico.TipoDocumentoId;
            });

            if (existeTipoDocumento) {
                errorp = true;
                utilidades.mensajeError("El documento " + tipoArchivoUnico.TipoDocumento + " ya se encuentra cargado en este paso. Si desea reemplazarlo elimine el anterior y vuelva a cargarlo.");
            }
                ...
                trasladosServicio.obtenerDatosUsuario($sessionStorage.usuario.permisos.IdUsuarioDNP, identidadVentanaEntidad,
                    ...
                    .then(function (respuesta) {
                        ...
                        for (var i = 0; i < input.files.length; i++) {
                            ...
                            //Se agrega version 1 a los documentos cargados en primera instancia
                            let versionDocumentoSoporte = 1;
                            ...
                            archivoServicios.cargarArchivo(archivo, vm.modelo.coleccion).then(function (response) {
                                if (response === undefined || typeof response === 'string') {
                                    ...
                                    var nombreDocumento = obtenerNombreDocumento(tipoArchivo.TipoDocumento);
                                    var prefijo = nombreDocumento && nombreDocumento.toLowerCase().startsWith('documento') ? 'El ' : 'El documento ';
                                    
                                    utilidades.mensajeSuccess('', false, false, false, prefijo + nombreDocumento + " versi&oacute;n " + versionDocumentoSoporte + ", se ha agregado con &eacute;xito.");
                                    cerrarModal();
                                    guardarCapituloModificado(1);
                                }
                            }, error => {
                                console.log(error);
                            });
        ...
        // Función para evitar redundancia con la palabra "documento"
        var obtenerNombreDocumento = function (tipoDocumento) {
            if (tipoDocumento && tipoDocumento.toLowerCase().startsWith('documento')) {
    ...
    angular.module('backbone').controller('cargarDocModalController', cargarDocModalController);
})();
