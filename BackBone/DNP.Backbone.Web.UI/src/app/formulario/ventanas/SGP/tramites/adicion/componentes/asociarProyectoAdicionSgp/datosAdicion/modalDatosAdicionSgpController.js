(function () {
    ...
    modalDatosAdicionSgpController.$inject = [
        '$sessionStorage', '$uibModalInstance', 'datosAdicionSgpServicio', 'utilidades', 'constantesBackbone', '$scope'
    ];

    function modalDatosAdicionSgpController(
        $sessionStorage, $uibModalInstance, datosAdicionSgpServicio, utilidades, constantesBackbone, $scope
    ) {
        ...
        function obtenerSectores() {
            ...
            datosAdicionSgpServicio.ObtenerSectores(parametros)
        ...
        async function cambioSector(entidadIdSeleccionado) {
            ...
            var listaEntidades = [];
            return datosAdicionSgpServicio.ObtenerListaEntidades(parametros, 1)
                .then(respuesta => {
                    if (!respuesta.data)
                        return;
        ...
        function guardar() {
           ...
           /* if (!vm.NombreDonante) {
            ...
            if (!vm.NumeroConvenio) {
                utilidades.mensajeError("Verifique el numero del Acuerdo/Convenio/Resolución.", false); return false;
            }
            
            if (!vm.ValorConvenio) {
                utilidades.mensajeError("Verifique el valor del Acuerdo/Convenio/Resolución.", false); return false;
            }
            else {
            ...
            if (!vm.ValorConvenioVigencia) {
                utilidades.mensajeError("Verifique el valor para la vigencia del Acuerdo/Convenio/Resolución.", false); return false;
            }
            else {
                ...
                if (valConvenioVig) {
                    if (valConvenioVig[0].length > 14) {
                        utilidades.mensajeError("Verifique el valor para vigencia del Acuerdo/Convenio/Resolución.", false); return false;
                    }
                }
            }
            //if (!vm.ValorConvenioVigencia) {
            ...
            if (!vm.Objeto) {
                utilidades.mensajeError("Verifique el valor de Objeto del Acuerdo/Convenio/Resolución.", false); return false;
            }
            ...
            if (vm.btnGuardarEditar == 'GUARDAR') {
                datosAdicionSgpServicio.GuardarDatosAdicionSgp(params, usuarioDNP)
                    .then(function (response) {
                ...
                utilidades.mensajeWarning("Si se realiza esta acción, todos los posibles datos diligenciados en la sección 'Datos de la adición' se perderán. Usted deberá verficar la información incluida previamente en otros espacios sea coherente con la modificación. Esta seguro de continuar?",
                    function funcionContinuar() {
                        ...
                        return datosAdicionSgpServicio.GuardarDatosAdicionSgp(params, usuarioDNP)
                            .then(function (response) {
                                let exito = response.data;
                                if (exito != undefined) {
                                    var respuesta = jQuery.parseJSON(exito);
    ...
    angular.module('backbone').controller('modalDatosAdicionSgpController', modalDatosAdicionSgpController)
