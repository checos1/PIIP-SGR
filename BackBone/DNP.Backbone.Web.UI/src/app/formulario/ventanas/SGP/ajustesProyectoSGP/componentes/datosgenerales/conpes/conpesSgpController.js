(function () {
	...
	function conpesSgpController(
		...
		vm.lang = "es";
		vm.conpes = "";
		vm.nombreComponente = "datosgeneralessgprelacionconlaplsgp";
		vm.guiMacroproceso = '';
		...
		vm.guardarConpes = function () {
			...
			conpesSgpServicio.AdicionarConpes(data)
		...
		//	conpesSgpServicio.CargarConpes(vm.idProyecto, vm.idInstancia, vm.guiMacroproceso, $sessionStorage.idNivel, $sessionStorage.idFlujoIframe)
		//		.then(resultado => {
		...
		vm.buscar = function (response) {
			...
			if (validarCampos()) {
				vm.consultoConpes = false;
				conpesSgpServicio.BuscarConpes(vm.conpes)
					.then(function (response) {
						vm.resultConpes = '';
						if (response.data) {
							vm.dataCONPES = null;
	...
	angular.module('backbone').component('conpesSgp', {

		templateUrl: "src/app/formulario/ventanas/SGP/ajustesProyectoSGP/componentes/datosgenerales/conpes/conpesSgp.html",
		controller: conpesSgpController,
		controllerAs: "vm",
		bindings: {
			guardadoevent: '&',
