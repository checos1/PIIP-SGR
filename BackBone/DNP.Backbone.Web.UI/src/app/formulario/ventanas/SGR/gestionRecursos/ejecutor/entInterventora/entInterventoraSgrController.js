(function () {
    ...
    entInterventoraSgrController.$inject = ['$scope'];

    function entInterventoraSgrController(
        $scope
    ) {
        var vm = this;
        vm.nombreComponente = "sgrejecutordesignacionentinterventor";
        vm.nombreComponenteHijo = "sgrejecutordesignacionentinterventorregistrovalorinterventor";
        ...
        vm.handlerComponentes = [
            { id: 1, componente: 'sgrejecutordesignacionentinterventorregistrovalorinterventor', handlerValidacion: null, handlerCambios: null, esValido: true },
            { id: 2, componente: 'sgrejecutordesignacionentinterventorregistrodesignacioninterventor', handlerValidacion: null, handlerCambios: null, esValido: true },
            { id: 3, componente: 'sgrejecutordesignacionentinterventorregistrovalorctei', handlerValidacion: null, handlerCambios: null, esValido: true },
            { id: 4, componente: 'sgrejecutordesignacionentinterventorregistrovalorsupervisor', handlerValidacion: null, handlerCambios: null, esValido: true }
        ...
        vm.inicializarComponenteCheck = function () {
            vm.handlerComponentesChecked = {
                'sgrejecutordesignacionentinterventorregistrovalorinterventor': true,
                'sgrejecutordesignacionentinterventorregistrodesignacioninterventor': true,
                'sgrejecutordesignacionentinterventorregistrovalorctei': true,
                'sgrejecutordesignacionentinterventorregistrovalorsupervisor': true
        ...
        */
        vm.notificacionValidacionEvent = function (listErrores) {
            var erroresList;
            erroresList = listErrores.errores.filter(p => p.Seccion == vm.nombreComponente);
            vm.inicializarComponenteCheck();
            vm.esValido = true;
            for (var i = 0; i < vm.handlerComponentes.length; i++) {
    ...
    angular.module('backbone').component('entInterventoraSgr', {
        templateUrl: "/src/app/formulario/ventanas/SGR/gestionRecursos/ejecutor/entInterventora/entInterventoraSgr.html",
        controller: entInterventoraSgrController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
