(function () {
    ...
    angular.module('backbone')
        .controller('modalAgregarInvolucradosGestionRecursosSgpController', modalAgregarInvolucradosGestionRecursosSgpController);

    modalAgregarInvolucradosGestionRecursosSgpController.$inject = [
        '$scope',
        '$uibModalInstance',
        '$sessionStorage',
        'utilidades',
    ...
    function modalAgregarInvolucradosGestionRecursosSgpController(
        $scope,
        $uibModalInstance,
        $sessionStorage,
        utilidades,
        ...
        function init() {
            ...
            transversalSgpServicio.SGPTransversalLeerParametro("RolFirmaGestionRecursosSGPPresupuesto")
                .then(function (respuestaParametro) {
                    if (respuestaParametro.data) {
                        var roles = $sessionStorage.listadoAccionesTramite.find(f => f.Ventana == 'GestionRecursosSGPPaso2');
                        if (roles != null && roles.Roles.length > 0) {
                            if (vm.involucrado.TipoRolConceptoId === 2) {
                                var rol = roles.Roles.find(f => f.NombreRol == 'R_Secretaría de planeación')
                                vm.IdRol = rol.IdRol;
                            }
                            else {
