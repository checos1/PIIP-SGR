(function () {
    ...
    costoActividadesController.$inject = ['$scope', 'costoActividadesServicio', '$sessionStorage', '$uibModal', 'utilidades', 'utilsValidacionSeccionCapitulosServicio', 'justificacionCambiosServicio'];

    function costoActividadesController(
        $scope,
        costoActividadesServicio,
        $sessionStorage,
        $uibModal,
        utilidades,
        ...
        function guardarCapituloModificado() {
            var data = {
                ...
                Justificacion: "",
                SeccionCapituloId: $('#id-capitulo-recursoscostosdelasacti').text(),
        ...
        function AgregarEntregable(productos, objectivo) {
            ...
            let modalInstance = $uibModal.open({
                ...
                templateUrl: 'src/app/formulario/ventanas/ajustes/componentes/recursos/costoActividades/modal/agregarEntregableModal.html',
                controller: 'agregarEntregableModalController',
                controllerAs: "vm",
                size: 'lg',
            ...
            modalInstance.result.then(data => {
                if (data != null) {
                    ...
                    costoActividadesServicio.AgregarEntregable(data, usuarioDNP).then(function (response) {
                        let exito = response.data;
                        if (exito) {
                            utilidades.mensajeSuccess("Operación realizada con éxito!", false, false, false);
                            vm.ObtenerResumenObjetivosProductosActividades(vm.BPIN);
        ...
        function EliminarEntregable(entregable) {
            ...
            utilidades.mensajeWarning('¿Se eliminará la línea de información. Los datos que allí haya incluido se perderán. Está seguro de eliminar?', function () {
                costoActividadesServicio.EliminarEntregable(entregable, usuarioDNP)
                    .then(function (response) {
                        let exito = response.data;
                        if (exito) {
                            utilidades.mensajeSuccess("Operación realizada con éxito!", false, false, false);
        ...
        vm.ObtenerResumenObjetivosProductosActividades = function (bpin, guardado = false) {
            ...
            costoActividadesServicio.ObtenerResumenObjetivosProductosActividades($sessionStorage.idInstancia).then(
    ...
    angular.module('backbone').component('costoActividades', {
        templateUrl: "src/app/formulario/ventanas/ajustes/componentes/recursos/costoActividades/costoActividades.html",
        controller: costoActividadesController,
        controllerAs: "vm",
        location: "es-COP",
        bindings: {
            callback: '&',
