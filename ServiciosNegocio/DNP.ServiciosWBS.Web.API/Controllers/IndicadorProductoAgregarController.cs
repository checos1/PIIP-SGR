namespace DNP.ServiciosWBS.Web.API.Controllers
{
    ...
    public class IndicadorProductoAgregarController : ApiController
    {

        private readonly IIndicadorProductoAgregarServicios _indicadorProductoAgregarServicios;
        private readonly IAutorizacionUtilidades _autorizacionUtilidades;

        public IndicadorProductoAgregarController(IIndicadorProductoAgregarServicios indicadorProductoAgregaServicios, IAutorizacionUtilidades autorizacionUtilidades)
        {
            _indicadorProductoAgregarServicios = indicadorProductoAgregaServicios;
            _autorizacionUtilidades = autorizacionUtilidades;
        }


        [Route("api/IndicadorProductoAgregar")]
        [SwaggerResponse(HttpStatusCode.OK, "Información de indicadores de productos para ser agregados", typeof(IndicadorProductoAgregarDto))]
        [HttpGet]
            ...
            try
            {
                var respuestaAutorizacion = _autorizacionUtilidades.
                                            ValidarUsuario(RequestContext.Principal.Identity.Name,
                                                           ...
                                                           ConfigurationManager.AppSettings["consultarIndicadorProductoAgregar"]).
                                            Result;
                ...
                var result =
                    await Task.Run(() => _indicadorProductoAgregarServicios.ObtenerIndicadorProductoAgregar(new ParametrosConsultaDto
                    {
                        Bpin =
                                                                                          bpin,
                        AccionId
                ...
                if (result != null) return Ok<IndicadorProductoAgregarDto>(result);
        ...
        [Route("api/IndicadorProductoAgregar/Preview")]
        [SwaggerResponse(HttpStatusCode.OK, "Información de indicadores de productos para ser agregados dummy", typeof(IndicadorProductoAgregarDto))]
        [HttpGet]
            ...
            try
            {
                var respuestaAutorizacion = _autorizacionUtilidades.
                                            ValidarUsuario(RequestContext.Principal.Identity.Name,
                                                           ...
                                                           ConfigurationManager.AppSettings
                                                               ["idAdministracionEnAutorizaciones"],
                                                           ConfigurationManager.AppSettings["previewIndicadorProductoAgregar"]).
                                            Result;

                if (!respuestaAutorizacion.IsSuccessStatusCode) return ResponseMessage(respuestaAutorizacion);

                var result = await Task.Run(() => _indicadorProductoAgregarServicios.ObtenerIndicadorProductoAgregarPreview());

                return Ok<IndicadorProductoAgregarDto>(result);
            }
            catch (AutorizacionException e)
            {
                throw new HttpResponseException(Request.CreateErrorResponse(HttpStatusCode.Unauthorized, e));
        ...
        [Route("api/IndicadorProductoAgregar/Temporal")]
        [SwaggerResponse(HttpStatusCode.OK, "Informacion devuelta por guardado temporal", typeof(HttpResponseMessage))]
        [HttpPost]
        public async Task<IHttpActionResult> Temporal(IndicadorProductoAgregarDto contenido)
        {
            try
            {
                var respuestaAutorizacion = _autorizacionUtilidades.
                                            ValidarUsuario(RequestContext.Principal.Identity.Name,
                                                           ...
                                                           ConfigurationManager.AppSettings
                                                               ["idAdministracionEnAutorizaciones"],
                                                           ConfigurationManager.AppSettings["postIndicadorProductoAgregarTemporal"]).
                                            Result;

                if (!respuestaAutorizacion.IsSuccessStatusCode) return ResponseMessage(respuestaAutorizacion);

                var parametrosGuardar = _indicadorProductoAgregarServicios.ConstruirParametrosGuardado(Request, contenido);
                ...
                await Task.Run((Action)(() => _indicadorProductoAgregarServicios.Guardar(parametrosGuardar, parametrosAuditoria, true)));
        ...
        [HttpPost]
        public async Task<IHttpActionResult> Definitivo([FromBody]IndicadorProductoAgregarDto proyecto)
        {
            try
            {
                var respuestaAutorizacion = _autorizacionUtilidades.
                                            ValidarUsuario(RequestContext.Principal.Identity.Name,
                                                           ...
                                                           ConfigurationManager.AppSettings
                                                               ["idAdministracionEnAutorizaciones"],
                                                           ConfigurationManager.AppSettings
                                                               ["postIndicadorProductoAgregarDefinitivo"]).
                                            Result;

                if (!respuestaAutorizacion.IsSuccessStatusCode) return ResponseMessage(respuestaAutorizacion);


                var parametrosGuardar = _indicadorProductoAgregarServicios.ConstruirParametrosGuardado(Request, proyecto);
                ...
                await Task.Run((Action)(() => _indicadorProductoAgregarServicios.Guardar(parametrosGuardar, parametrosAuditoria, false)));
