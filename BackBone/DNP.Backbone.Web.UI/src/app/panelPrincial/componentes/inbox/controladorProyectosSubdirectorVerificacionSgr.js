...
(function () {
    'use strict';
    angular.module('backbone').controller('proyectosSubdirectorVerificacionSgrController', proyectosSubdirectorVerificacionSgrController);

    proyectosSubdirectorVerificacionSgrController.$inject = [
        '$scope',
    ...
    function proyectosSubdirectorVerificacionSgrController(
        $scope,
        servicioPanelPrincipal,
        $uibModal,
        $log,
        ...
        vm.abrirEnviarSubPaso = abrirEnviarSubPaso;
        ...
        vm.proyectoFiltro = {
            ...
            IdsEtapas: [constantesBackbone.idEtapaViabilidadRegistro],
            IdsSubPasosVerificacion: [constantesBackbone.idSubPasoSubdirectorOcadPazSgr],
            procesoId: null,
        ...
        vm.verPdf = function (row) {
            transversalSgrServicio.SGR_Transversal_LeerParametro("VerificacionOcadPazNivelId")
                .then(function (respuestaParametro) {
                    ...
                    const borrador = true;
                    const projectId = row.IdObjetoNegocio;
                    ...
                    servicioFichasProyectos.ObtenerIdFicha(nombreFichaTramite)
                        .then(function (respuestaFicha) {
                            var fichaPlantilla = {
                                ...
                                PARAM_BORRADOR: true,
                                PARAM_BPIN: row.IdObjetoNegocio,
                                InstanciaId: row.IdInstancia,
                                NivelId: respuestaParametro.data.Valor,
                                TramiteId: row.IdObjetoNegocio
                            };
        ...
        function abrirEnviarSubPaso(row, avance) {
            var mensajeNotificacion = null;
            var mensajeExito = null;
            if (avance === 0) {
                mensajeNotificacion = vm.mensajeDevolucion;
                mensajeExito = vm.mensajeExitoDevolucion;
            } else if (avance === -1) {
                mensajeNotificacion = vm.mensajeDevolucionPaso;
                mensajeExito = vm.mensajeExitoDevolucionPaso;
            } else if (avance === 1 || avance === 2) {
                mensajeNotificacion = vm.mensajeAvance;
                mensajeExito = vm.mensajeExitoAvance;
            }
            if (avance === 0) {

                var modalInstance = $uibModal.open({
                    animation: $scope.animationsEnabled,
                    templateUrl: '/src/app/panelPrincial/modales/subpasoOcad/observacionModal.html',
                    controller: 'observacionModalController',
                    controllerAs: 'vm',
                    openedClass: "modal-observacionDevolucionDNP",
                    resolve: {
                        avance: () => avance,
                        instancia: () => row.IdInstancia,
                        accion: () => row.IdAccion,
                ...
                utilidades.mensajeWarning("¿Está seguro de continuar?", function funcionContinuar() {
                    ValidarExistenciaFirmaUsuario($sessionStorage.usuario.permisos.IdUsuarioDNP, row.IdObjetoNegocio, row.IdRol, row.IdInstancia, row.IdAccion, avance, mensajeExito);
                }, function funcionCancelar(reason) {
                    return;
                }, null, null, mensajeNotificacion);
            }
        }

        function ValidarExistenciaFirmaUsuario(usuario, idObjetoNegocio, idRol, idInstancia, idAccion, avance, mensajeExito) {
            return firmaFormularioSgrServicio.validarSiExisteFirmaUsuario(usuario)
                .then(function (response) {
                    if (response.data && response.data.Exito) {
                        ...
                        GenerarFichaVerificacionSgr(idInstancia, idObjetoNegocio, idRol, dataSubPaso, mensajeExito);
                    } else {
                        utilidades.mensajeError(vm.mensajeNoExisteFirmaSubdirector, null, "Señor usuario");
                    }
                })
                .catch(error => {
                    if (error.status == 400) {
        ...
        function GenerarFichaVerificacionSgr(instanciaId, objetoNegocioId, rolId, dataSubPaso, mensajeExito) {
            var data = {
                InstanciaId: instanciaId,
                ObjetoNegocioId: objetoNegocioId,
                IdRol: rolId,
                NivelId: '00000000-0000-0000-0000-000000000000'
            }

            return requisitosPazSgrServicio.SGR_OCADPaz_GenerarFichaVerificacion(data)
                .then(function (response) {
                    if (response.status === 200) {
                        EjecutarSubPaso(dataSubPaso, mensajeExito);
                    }
                })
                .catch(error => {
                    if (error.status == 400) {
        ...
        function EjecutarSubPaso(dataSubPaso, mensajeExito) {
            return flujoServicios.Flujos_SubPasoEjecutar(dataSubPaso)
                .then(function (response) {
                    if (response.data && response.status === 200) {
                        utilidades.mensajeSuccess("Ahora usted puede acceder a este proceso desde la consola de procesos.",
                            true,
            ...
            function exitoCantidad(respuesta) {
                if (respuesta.data != null) {
                    ...
                    var dataE = vm.macroProcesosCantidad.filter(x => (String(x.NivelPadreId).toUpperCase() == constantesBackbone.idEtapaNuevaEjecucion ||
                        ...
                        String(x.NivelPadreId).toUpperCase() == constantesBackbone.idEtapaSubdirector) && x.TipoObjeto == 'Proyecto');
        ...
        function obtenerInbox(idObjeto) {
            ...
            if (!vm.tipoEntidad) {
                vm.tipoEntidad = constantesAutorizacion.tipoEntidadTerritorial;
            }
            if (!vm.proyectoFiltro.tipoEntidad) {
                vm.proyectoFiltro.tipoEntidad = constantesAutorizacion.tipoEntidadTerritorial;
            }
            return cargarEntidades(idObjeto).then(function () {

                vm.listaTipoEntidad.forEach(item => {
                    if (item.Nombre !== constantesAutorizacion.tipoEntidadTerritorial) {
                ...
                mostrarMensajeRespuesta();
                cargarDatosVerificacionOcadPaz();
            });
        };

        function cargarDatosVerificacionOcadPaz() {
            transversalSgrServicio.SGR_Transversal_LeerParametro("MensajeDevolucionTecnicoVerificacionSGR")
                .then(function (respuestaParametro) {
                    if (respuestaParametro.data) {
                        vm.mensajeDevolucion = respuestaParametro.data.Valor;
                    }
                }, function (error) {
                    utilidades.mensajeError(error);
                });

            transversalSgrServicio.SGR_Transversal_LeerParametro("MensajeDevolverPreviosVerificacionSGR")
                .then(function (respuestaParametro) {
                    if (respuestaParametro.data) {
                        vm.mensajeDevolucionPaso = respuestaParametro.data.Valor;
                    }
                }, function (error) {
                    utilidades.mensajeError(error);
                });

            transversalSgrServicio.SGR_Transversal_LeerParametro("MensajeAvanceViabilidadVerificacionSGR")
                .then(function (respuestaParametro) {
                    if (respuestaParametro.data) {
                        vm.mensajeAvance = respuestaParametro.data.Valor;
                    }
                }, function (error) {
                    utilidades.mensajeError(error);
                });

            transversalSgrServicio.SGR_Transversal_LeerParametro("MensajeExitoDevolucionTecnicoVerificacionSGR")
                .then(function (respuestaParametro) {
                    if (respuestaParametro.data) {
                        vm.mensajeExitoDevolucion = respuestaParametro.data.Valor;
                    }
                }, function (error) {
                    utilidades.mensajeError(error);
                });

            transversalSgrServicio.SGR_Transversal_LeerParametro("MensajeExitoDevolverPreviosVerificacionSGR")
                .then(function (respuestaParametro) {
                    if (respuestaParametro.data) {
                        vm.mensajeExitoDevolucionPaso = respuestaParametro.data.Valor;
                    }
                }, function (error) {
                    utilidades.mensajeError(error);
                });

            transversalSgrServicio.SGR_Transversal_LeerParametro("MensajeExitoAvanceViabilidadVerificacionSGR")
                .then(function (respuestaParametro) {
                    if (respuestaParametro.data) {
                        vm.mensajeExitoAvance = respuestaParametro.data.Valor;
                    }
                }, function (error) {
                    utilidades.mensajeError(error);
                });

            transversalSgrServicio.SGR_Transversal_LeerParametro("MensajeNoFirmaSubdirectorVerificacionSGR")
                .then(function (respuestaParametro) {
                    if (respuestaParametro.data) {
                        vm.mensajeNoExisteFirmaSubdirector = respuestaParametro.data.Valor;
                    }
                }, function (error) {
                    utilidades.mensajeError(error);
                });
        ...
        function cargarEntidades(idObjeto) {
            ...
            return servicioPanelPrincipal.obtenerInboxVerificacionSgr(vm.peticionObtenerInbox, vm.proyectoFiltro, columnasVisibles).then(
                function (respuesta) {
                    vm.cantidadDeProyectos = 0;
                    vm.listaEntidades = [];
                            ...
                            grupoEntidade.ListaEntidades.forEach(entidad => {
                                ...
                                entidad.ObjetosNegocio.forEach(negocio => {
                                    ...
                                    vm.listaDatos.push({
                                        ...
                                        UsuarioEncargadoOcadPazId: negocio.UsuarioEncargadoOcadPazId,
                                        UsuarioEncargadoAsignadoOcadPaz: negocio.UsuarioEncargadoAsignadoOcadPaz,
                                        IdRol: negocio.IdRol,
                                        AsignadoA: negocio.AsignadoA,
                                        CorreoAsignadoA: negocio.CorreoAsignadoA,
                                        Cumple: negocio.Cumple
                                    });
                                });
        ...
        function getIdEtapa() {
            ...
            switch (vm.etapa) {
                ...
                case 'gr':
                    ...
                    break;
                case 'ej':
                    idEtapa = [constantesBackbone.idEtapaNuevaEjecucion, constantesBackbone.idEtapaProgramacionEjecucion, constantesBackbone.idEtapaAjustesEjecucion, constantesBackbone.idEtapaTramitesEjecucion, constantesBackbone.idEtapaSubdirector];
                    break;
                case 'se':
                    idEtapa = [];
                    break;
        ...
        function mostrarModal(flujoSeleccionado) {
            ...
            function crearModal(entidades) {
                ...
                if (flujoSeleccionado.TipoObjeto.Id == idTipoProyecto) {
                ...
                function (resultado) {
                    ...
                    if (cantidadInstanciasFallidas) {
                        ...
                        if (resultado != null & resultado[0].NumeroTramite != null)
                            utilidades.mensajeSuccess('Se crearon instancias exitosamente.<br/>Proceso No. ' + resultado[0].NumeroTramite, false, '', '', 'El proceso fue creado con éxito.');
                        else
                            utilidades.mensajeSuccess('Se crearon instancias exitosamente.<br/>Proceso No. ', false, '', '', 'El proceso fue creado con éxito.');
                        vm.cargarEntidades('');
                    }
                },
                function (error) {
    ...
    angular.module('backbone')
        .component('proyectosSubdirectorVerificacionSgr', {
            templateUrl: "/src/app/panelPrincial/componentes/inbox/proyectosSubdirectorVerificacionSgr.template.html",
            controller: 'proyectosSubdirectorVerificacionSgrController',
            controllerAs: 'vm'
        });
