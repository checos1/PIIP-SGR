(function () {
    ...
    informacionPresupuestalFormulario.$inject = [
        '$scope',
        '$sessionStorage',
        'utilidades',
        'constantesBackbone',
    ...
    function informacionPresupuestalFormulario(
        $scope,
        $sessionStorage,
        utilidades,
        constantesBackbone,
                ...
                item.ListaFuentes.map(function (itemfuente) {
                    ...
                    if (fuenteIndex < 0) {
                        ...
                        if (fuente.GrupoRecurso === 'N') {
                            vm.listaProyectos[proyectoIndex].totalCSFNacion = ConvertirNumero(sumar(vm.listaProyectos[proyectoIndex].totalCSFNacion,fuente.ValorContracreditoCSF));
                            vm.listaProyectos[proyectoIndex].totalNacion = ConvertirNumero(sumar(vm.listaProyectos[proyectoIndex].totalNacion,fuente.ValorContracreditoCSF));

                        }
                        else if (fuente.GrupoRecurso === 'P') {
                            vm.listaProyectos[proyectoIndex].totalCSFPropios = ConvertirNumero(sumar(vm.listaProyectos[proyectoIndex].totalCSFPropios, fuente.ValorContracreditoCSF));
                            vm.listaProyectos[proyectoIndex].totalPropios = ConvertirNumero(sumar(vm.listaProyectos[proyectoIndex].totalPropios, fuente.ValorContracreditoCSF));
                        }

                        /*suma lo soclitado segun origen*/
                        if (fuente.GrupoRecurso === 'N') {
                            vm.listaProyectos[proyectoIndex].totalSSFNacion = ConvertirNumero(sumar(vm.listaProyectos[proyectoIndex].totalSSFNacion, fuente.ValorContracreditoSSF));
                            vm.listaProyectos[proyectoIndex].totalNacion = ConvertirNumero(sumar(vm.listaProyectos[proyectoIndex].totalNacion, fuente.ValorContracreditoSSF));
                        }
                        else if (fuente.GrupoRecurso === 'P') {
                            vm.listaProyectos[proyectoIndex].totalSSFPropios = ConvertirNumero(sumar(vm.listaProyectos[proyectoIndex].totalSSFPropios, fuente.ValorContracreditoSSF));
                            vm.listaProyectos[proyectoIndex].totalPropios = ConvertirNumero(sumar(vm.listaProyectos[proyectoIndex].totalPropios, fuente.ValorContracreditoSSF));
                        }
                ...
                if (tramiteFuentePrespuestalindex < 0) {
                    ...
                    if (datosAGuardar.Operacion === 'Contracredito') {
                        ...
                        if (item.ValorContracreditoSSF.toString().includes('.'))
                            valortmpSSF = item.ValorContracreditoSSF.toString().replaceAll('.', '');
                        else
                            valortmpSSF = item.ValorContracreditoSSF;
                        ...
                        ValorVigentetmpCSF = parseFloat(ValorVigentetmpCSF);

                        if (item.ValorVigenteSSF.toString().includes('.'))
                            ValorVigentetmpSSF = item.ValorVigenteSSF.toString().replaceAll('.', '');
                        else
                            ValorVigentetmpSSF = item.ValorVigenteSSF;
                        ValorVigentetmpSSF = parseFloat(ValorVigentetmpSSF);

                        if (valortmpCSF > ValorVigentetmpCSF) {
                            hayerror = true;
                            mensajeError = 'El valor solicitado CSF no puede ser mayor al valor vigente  CSF ';
                        }

                        if (valortmpSSF > ValorVigentetmpSSF) {
                            hayerror = true;
                            mensajeError = 'El valor solicitado SSF no puede ser mayor al valor vigente  SSF ';
                        }
                    }
                    if (!hayerror) {
                        if (item.GrupoRecurso === 'N') {
                            sumaTemporalNacion = ConvertirNumero(sumar(sumaTemporalNacion, sumar(item.ValorContracreditoCSF, item.ValorContracreditoSSF)));
                        }
                        else if (item.GrupoRecurso === 'P') {
                            sumaTemporalPropios = ConvertirNumero(sumar(sumaTemporalPropios, sumar(item.ValorContracreditoCSF, item.ValorContracreditoSSF)));
                        }
                        tramiteFuentePrespuestal.ValorContracreditoCSF = limpiaNumero(item.ValorContracreditoCSF);
                        tramiteFuentePrespuestal.ValorContracreditoSSF = limpiaNumero(item.ValorContracreditoSSF)
                        ...
                        if (parseFloat(limpiaNumero(sumaTemporalNacion)) > parseFloat(limpiaNumero(datosAGuardar.ValorTotalNacion))) {
                            hayerror = true;
                            mensajeError = 'Suma del valor solicitado nación de las fuentes en el proyecto no puede ser mayor al valor total nación ';
                        }
    ...
    angular.module('backbone').component('informacionPresupuestalFormulario', {
        restrict: 'E',
        transclude: true,
        scope: {
            modelo: '='
        },
        templateUrl: "src/app/formulario/ventanas/comun/informacionPresupuestal/informacionPresupuestalFormulario.html",
        controller: informacionPresupuestalFormulario,
        controllerAs: "vm",
        bindings: {
            callback: '&',
