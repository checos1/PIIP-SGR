(function () {
    ...
    aprobacionincorporacionController.$inject = [
        '$sessionStorage',
        '$scope',
        'aprobacionincorporacionServicio',
        'utilidades',
    ...
    function aprobacionincorporacionController(
        $sessionStorage,
        $scope,
        aprobacionincorporacionServicio,
        utilidades,
        ...
        vm.habilitaBotonesGuardar = true;
            ...
            , (newVal, oldVal) => {
                if (newVal) {
                    if ($sessionStorage.usuario.roles.find(x => x.Nombre.includes('Analista'))) {
                        vm.habilitaBotonesGuardar = !$sessionStorage.soloLectura ? true : false; 
                    }
                    else
                        vm.habilitaBotonesGuardar = false;
                }
            }, true);
        
        $scope.$watch('vm.tramiteid', function () {
            if (vm.tramiteid !== '' && vm.tramiteid !== undefined && vm.tramiteid !== null) {
                if ($sessionStorage.tramiteId)
                    ObteneraprobacionincorporacionVigenciaFutura();
            }
        });
        ...
        function ObteneraprobacionincorporacionVigenciaFutura() {
            return aprobacionincorporacionServicio.ObtenerPresupuestalProyectosAsociados(vm.tramiteid, vm.instanciaId).then(
                function (respuesta) {
                    //debugger;
        ...
        function CancelarAsociados(asociados) {
            utilidades.mensajeWarning("¿Está seguro de continuar?", function funcionContinuar() {
                ...
                angular.forEach(asociados.DetalleFuentes, function (series) {
                    series.ValorIncorporarAprobadoCSF = series.ValorIncorporarAprobadoCSFOriginal;
                    series.ValorIncorporarAprobadoSSF = series.ValorIncorporarAprobadoSSFOriginal;
                });
                ...
                angular.forEach(asociados.DetalleFuentes.filter(x => x.NombreTipoRecurso.includes('Nación')), function (series) {
                    TotalSolicitadoNacion = TotalSolicitadoNacion + parseFloat(series.ValorIncorporarAprobadoCSF) + parseFloat(series.ValorIncorporarAprobadoSSF);
                });

                var proyecto = $scope.datos.ResumenProyectos[credito].Proyectos.findIndex(x => x.ProyectoId == asociados.ProyectoId);

                $scope.datos.ResumenProyectos[credito].Proyectos[proyecto].TotalAprobadoNacion = TotalSolicitadoNacion;
                ...
                angular.forEach(asociados.DetalleFuentes.filter(x => x.NombreTipoRecurso.includes('Propios')), function (series) {
                    TotalSolicitadoPropios = TotalSolicitadoPropios + parseFloat(series.ValorIncorporarAprobadoCSF) + parseFloat(series.ValorIncorporarAprobadoSSF);
                });
                $scope.datos.ResumenProyectos[credito].Proyectos[proyecto].TotalAprobadoPropios = TotalSolicitadoPropios;

                $scope.datos.ResumenProyectos[credito].totalTipoAprobacion = TotalSolicitadoNacion + TotalSolicitadoPropios;
        ...
        function GuardarAsociados(asociados) {
            var valida = false;
            let fuentes = [];
            var TotalValorAprobarCSF = 0;
            var TotalValorAprobarSSF = 0;
            angular.forEach(asociados.DetalleFuentes, function (series) {
                let c = {
                    TramiteId: vm.TramiteId,
                    ProyectoId: asociados.ProyectoId,
                    EntidadId: $sessionStorage.EntidadId === undefined ? 0 : $sessionStorage.EntidadId,
                    FuenteId: series.TipoRecursoId,
                    TipoAccion: 'C',
                    ValorAprobadoCSF: series.ValorIncorporarAprobadoCSF,
                    ValorAprobadoSSF: series.ValorIncorporarAprobadoSSF
                };
                fuentes.push(c);
                TotalValorAprobarCSF = TotalValorAprobarCSF + series.ValorIncorporarAprobadoCSF;
                TotalValorAprobarSSF = TotalValorAprobarSSF + series.ValorIncorporarAprobadoSSF;
                
                if ((series.ValorIncorporarAprobadoCSF > series.ValorIncorporarCSF) || (series.ValorIncorporarAprobadoSSF > series.ValorIncorporarSSF)) {
                    valida = true;
                }
            });

            if (valida) {
                utilidades.mensajeError("El valor aprobado debe ser igual o menor al valor a incorporar");
                return;
            }

            if (TotalValorAprobarCSF == 0 && TotalValorAprobarSSF == 0) {
            ...
            return comunesServicio.guardarFuentesTramiteProyectoAprobacion(fuentes).then(
                function (respuesta) {
                    if (respuesta.data && (respuesta.statusText === "OK" || respuesta.status === 200)) {
                        ...
                        angular.forEach(asociados.DetalleFuentes, function (series) {
                            series.ValorIncorporarAprobadoCSFOriginal = series.ValorIncorporarAprobadoCSF;
                            series.ValorIncorporarAprobadoSSFOriginal = series.ValorIncorporarAprobadoSSF;
                        });
        ...
        function CancelarAportantes(aportantes) {
            utilidades.mensajeWarning("¿Está seguro de continuar?", function funcionContinuar() {
                ...
                $scope.datos.ResumenProyectos[contrato].Proyectos[proyecto].TotalAprobadoNacion = TotalSolicitadoNacion;

                var TotalSolicitadoPropios = 0;
                angular.forEach(aportantes.DetalleFuentes.filter(x => x.NombreTipoRecurso.includes('Propios')), function (series) {
                    TotalSolicitadoPropios = TotalSolicitadoPropios + parseFloat(series.ValorIncorporarAprobadoCSF) + parseFloat(series.ValorIncorporarAprobadoSSF);
                });
                $scope.datos.ResumenProyectos[contrato].Proyectos[proyecto].TotalAprobadoPropios = TotalSolicitadoPropios;

                $scope.datos.ResumenProyectos[contrato].totalTipoAprobacion = TotalSolicitadoNacion + TotalSolicitadoPropios;
        ...
        function GuardarAportantes(aportantes) {
            ...
            angular.forEach(aportantes.DetalleFuentes, function (series) {
                let c = {
                    TramiteId: vm.TramiteId,
                    ProyectoId: aportantes.ProyectoId,
                    EntidadId: $sessionStorage.EntidadId === undefined ? 0 : $sessionStorage.EntidadId,
                    FuenteId: series.TipoRecursoId,
                    TipoAccion: 'C',
                    ValorAprobadoCSF: series.ValorIncorporarAprobadoCSF,
    ...
    angular.module('backbone').component('aprobacionincorporacion', {
        templateUrl: "src/app/formulario/ventanas/tramiteIncorporacion/componentes/informacionPresupuestalInc/aprobacionincorporacion/aprobacionincorporacion.html",
        controller: aprobacionincorporacionController,
        controllerAs: "vm",
        bindings: {
            guardadoevent: '&',
            notificacioncambios: '&',
