(function () {
    ...
    focalizacionconceptoController.$inject = ['$scope', '$sessionStorage', 'utilidades',
        'constantesBackbone','focalizacionAjustesSgrServicio', 'serviciosComponenteNotificaciones', 'trasladosServicio', 'solicitarconceptoServicio','sesionServicios'
    ];
    ...
    function focalizacionconceptoController(
        ...
        vm.ObtenerResumenSolicitudConcepto = function (bpin) {
            return focalizacionAjustesSgrServicio.ObtenerResumenSolicitudConcepto(bpin, usuarioDNP, $sessionStorage.idNivel).then(
                function (respuesta) {
        ...
        vm.ObtenerDireccionesTecnicasPoliticas = function () {
            ...
            return focalizacionAjustesSgrServicio.ObtenerDireccionesTecnicasPoliticasFocalizacion(usuarioDNP, $sessionStorage.idNivel).then(
                function (respuesta) {
                    if (respuesta.data != null && respuesta.data != "") {
                        var arreglolistas = jQuery.parseJSON(respuesta.data);
        ...
        vm.ObtenerPoliticasSolicitudConcepto = function (bpin) {
            return focalizacionAjustesSgrServicio.ObtenerPoliticasSolicitudConcepto(bpin, usuarioDNP, $sessionStorage.idNivel).then(
                function (respuesta) {
                    if (respuesta.data != null && respuesta.data != "") {
                        var arreglolistas = jQuery.parseJSON(respuesta.data);
        ...
        vm.AbrilNivel = function (politicaId, indexpoliticas) {
            ...
            vm.Politicas.PoliticaPrincipal.forEach(pol => {
                if (pol.PoliticaId == politicaId) {
                    if (pol.Descripcion != "") {
                        document.getElementById("btnGuardarPolitica-" + pol.PoliticaId).classList.add('disabled');
                        document.getElementById("descripcionconcepto-" + pol.PoliticaId).classList.add('disabled');
                    }
                    else {
                        document.getElementById("btnGuardarPolitica-" + pol.PoliticaId).classList.remove('disabled');
                        document.getElementById("descripcionconcepto-" + pol.PoliticaId).classList.remove('disabled');
                    }
                }
            });
        ...
        vm.guardarDT = function (politicaId) {
            ...
            var descripcion = document.getElementById("descripcionconcepto-" + politicaId).value;
            ...
            utilidades.mensajeWarning("¿Esta seguro de continuar?", function funcionContinuar() {
                ...
                focalizacionAjustesSgrServicio.ObtenerUsuariosRValidadorPoliticaTransversal(vm.peticion)
                    .then(resultado => {
                        vm.FocalizacionSolicitarConcepto = [];
                        ...
                        if (vm.analistas.length > 0) {
                            ...
                            focalizacionAjustesSgrServicio.FocalizacionSolicitarConceptoDT(vm.FocalizacionSolicitarConcepto).then(function (response) {
                                if (response.statusText == "OK") {
                                    ...
                                    document.getElementById("descripcionconcepto-" + politicaId).classList.add('disabled');
                                }
                                else {
