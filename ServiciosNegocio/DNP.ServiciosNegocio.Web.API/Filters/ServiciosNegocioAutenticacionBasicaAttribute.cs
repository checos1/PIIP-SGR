...
#pragma warning restore 1998
        {
            ...
            if (!httpRequest.Headers.Contains("Application-User"))
            {
                return null;
            }

            var user = httpRequest.Headers.GetValues("Application-User").First();

            var appClaims = new List<Claim> { new Claim(ClaimTypes.Name, user) };
            var identity = new ClaimsIdentity(appClaims, DefaultAuthenticationTypes.ApplicationCookie);
