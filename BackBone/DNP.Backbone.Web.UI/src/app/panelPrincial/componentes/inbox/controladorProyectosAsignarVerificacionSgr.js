...
(function () {
    'use strict';
    angular.module('backbone').controller('proyectosAsignarVerificacionSgrController', proyectosAsignarVerificacionSgrController);

    proyectosAsignarVerificacionSgrController.$inject = [
        '$scope',
    ...
    function proyectosAsignarVerificacionSgrController(
        $scope,
        servicioPanelPrincipal,
        $uibModal,
        $log,
        ...
        vm.abrirAsignacionTecnico = abrirAsignacionTecnico;
        ...
        vm.proyectoFiltro = {
            ...
            IdsSubPasosVerificacion: [constantesBackbone.idSubPasoAsignacionOcadPazSgr, constantesBackbone.idSubPasoTecnicoOcadPazSgr],
            procesoId: null,
        ...
        function abrirAsignacionTecnico(row) {


            var modalInstance = $uibModal.open({
                animation: $scope.animationsEnabled,
                templateUrl: 'src/app/formulario/ventanas/SGR/ocadPaz/verificacion/asignacion/modalAsignarTecnicoSgr.html',
                controller: 'modalAsignarTecnicoSgrController',
                controllerAs: "vm",
                openedClass: "modal-contentDNP",
                resolve: {
                    RolFiltroId: () => vm.IdRolTecnico,
                    EntidadId: () => vm.EntidadOcadPaz,
                    ProyectoId: () => row.ProyectoId,
                    InstanciaId: () => row.IdInstancia,
                    AccionId: () => row.IdAccion,
                    UsuarioEncargadoOcadPazId: () => row.UsuarioEncargadoOcadPazId,
                    UsuarioEncargadoAsignado: () => row.UsuarioEncargadoAsignadoOcadPaz
                }
            });
        ...
        function cargarDatosVerificacionOcadPaz() {
            transversalSgrServicio.SGR_Transversal_LeerParametro("RolTecnicoVerificacionOCADPaz")
                .then(function (respuestaParametro) {
                    if (respuestaParametro.data) {
                        vm.IdRolTecnico = respuestaParametro.data.Valor;
                    }
                }, function (error) {
                    utilidades.mensajeError(error);
                });

            transversalSgrServicio.SGR_Transversal_LeerParametro("EntidadOCADPaz")
                .then(function (respuestaParametro) {
                    if (respuestaParametro.data) {
                        vm.EntidadOcadPaz = respuestaParametro.data.Valor;
                    }
                }, function (error) {
                    utilidades.mensajeError(error);
                });
    ...
    angular.module('backbone')
        .component('proyectosAsignarVerificacionSgr', {
            templateUrl: "/src/app/panelPrincial/componentes/inbox/proyectosAsignarVerificacionSgr.template.html",
            controller: 'proyectosAsignarVerificacionSgrController',
            controllerAs: 'vm'
        });
