(function () {
    ...
    inclusionProyectosPrController.$inject = ['$scope', '$sessionStorage', 'comunesServicio'];

    function inclusionProyectosPrController(
        $scope,
        $sessionStorage,
        comunesServicio
    ) {
        var vm = this;
        vm.lang = "es";
        vm.nombreComponente = "distribucion";
        vm.nombreComponenteHijo = "distribuciondistribucionrecursos";
        vm.notificacionCambiosCapitulos = null;
        /*vm.modificoDatos = '0';*/


        vm.handlerComponentes = [
            { id: 1, componente: 'distribuciondistribucionrecursos', handlerValidacion: null, handlerCambios: null, esValido: true },
        ];
        vm.handlerComponentesChecked = {};

        //Inicio
        vm.initDistribucion = function () {
            ...
            vm.verificacalendario({ idEntidad: vm.IdEntidad, idNivel: vm.IdNivel, idSeccionCapitulo: vm.IdSeccionCapitulo });
        };


        /*-------------------------obtener validar calendario-------------------------------*/


        vm.IdNivel = undefined;
        vm.IdEntidad = undefined;
        vm.IdSeccionCapitulo = undefined;
        $scope.$watch('$sessionStorage.idNivel', function () {
            if ($sessionStorage.idNivel !== '' && $sessionStorage.idNivel !== undefined && $sessionStorage.idNivel !== null) {
                vm.IdNivel = $sessionStorage.idNivel;
                validarVariables();
            }
        });

        $scope.$watch('vm.IdSeccionCapitulo', function () {
            if (vm.IdSeccionCapitulo !== '' && vm.IdSeccionCapitulo !== undefined && vm.IdSeccionCapitulo !== null) {
                validarVariables();
            }
        });

        $scope.$watch('$sessionStorage.idEntidad', function () {
            if ($sessionStorage.idEntidad !== '' && $sessionStorage.idEntidad !== undefined && $sessionStorage.idEntidad !== null) {
        ...
        function validarVariables() {
            if (vm.IdNivel !== undefined && vm.IdEntidad !== undefined && vm.IdSeccionCapitulo !== undefined)
                vm.verificacalendario({ idEntidad: vm.IdEntidad, idNivel: vm.IdNivel, idSeccionCapitulo: vm.IdSeccionCapitulo, seccion: 'distribucion' });
        }
       ...
       * */
        ...
        vm.inicializarComponenteCheck = function () {
            vm.handlerComponentesChecked = {
                'distribuciondistribucionrecursos': true
            };
        }
        ...
        */
        ...
        vm.capitulos = function (listadoCapitulos) {
            ...
            if (listadoCapRecursos !== undefined && listadoCapRecursos !== null) {
                if (listadoCapRecursos.length > 0) {
    ...
    angular.module('backbone').component('inclusionProyectosPr', {
        templateUrl: "/src/app/formulario/ventanas/tramiteProgramacionRecursos/Componentes/inclusionProyectosPr/inclusionProyectosPr.html",
        controller: inclusionProyectosPrController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
