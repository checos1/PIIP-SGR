(function () {
    ...
    agregarSectoresCtusISgrController.$inject = ['$scope', 'requisitosSgrServicio', 'justificacionCambiosServicio','utilidades', '$sessionStorage'];

    function agregarSectoresCtusISgrController(
        $scope,
        ...
        var vm = this;
        vm.nombreComponente = "sgrctusintegradoelaboraciondatosverificacionagregarsectores";
        vm.Bpin = null;
        ...
        vm.componentesRefresh = [
            ...
            "sgrctusintegradoelaboraciondatosverificacionagregarsectores"
        ];
        ...
        vm.leerAcuerdoProyecto = function () {
            requisitosSgrServicio.SGR_Acuerdo_LeerProyecto(vm.proyectoId, vm.nivelId)
        ...
        vm.AgregarRegistro = function () {
            if ((parseInt(vm.Acuerdos.AcuerdoId) > 0) && (parseInt(vm.Sectores.AcuerdoSectorId) > 0) && (parseInt(vm.Clasificadores.AcuerdoSectorClasificadorId) > 0))
            {
        ...
        function actualizar(data) {
            return requisitosSgrServicio.SGR_Acuerdo_GuardarProyecto(data).then(function (response) {
                if (response.data) {
            ...
            utilidades.mensajeWarning("La línea de información seleccionada será desagregada de la tabla. Está seguro de continuar?",
                function funcionContinuar() {
                    return requisitosSgrServicio.SGR_Acuerdo_GuardarProyecto(data).then(function (response) {
                        if (response.data) {
                            guardarCapituloModificado();
                            utilidades.mensajeSuccess("Se ha desagregado con éxito la línea de información", false, false, false);
                            vm.Sectores.AcuerdoSectorId = '';
                            vm.Clasificadores.AcuerdoSectorClasificadorId = 0;
                            vm.leerAcuerdoProyecto();
                        } else {
    ...
    angular.module('backbone').component('agregarSectoresCtusISgr', {
        templateUrl: "/src/app/formulario/ventanas/SGR/ctusIntegrado/elaboracion/datosVerificacion/agregarSectores/agregarSectoresCtusISgr.html",

        controller: agregarSectoresCtusISgrController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
