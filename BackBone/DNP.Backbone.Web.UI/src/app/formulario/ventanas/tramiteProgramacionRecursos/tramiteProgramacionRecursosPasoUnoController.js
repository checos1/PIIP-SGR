(function () {
    ...
    tramiteProgramacionRecursosPasoUnoController.$inject = [
        '$sessionStorage',
        '$scope',
    ...
    function tramiteProgramacionRecursosPasoUnoController(
        $sessionStorage,
        $scope,
        tramiteVigenciaFuturaServicio,
        constantesBackbone,
        ...
        vm.listaSeccioCapitulo = [];
        vm.modificoFuentes = '0';
        vm.modificoDatos = '0';
        vm.mostrarMesaje = false;
        vm.textoMensaje = undefined;
        //vm.seccionActiva = 'distribucion';
        ...
        vm.verificaCalendario = verificaCalendario;
        ...
        vm.calendarioDistribucionCuotas = 'false';
        vm.modificoDatos = '0';
        ...
        $scope.$watch('vm.modificoDatos', function () {
            if (vm.modificoDatos !== '0' && vm.modificoDatos !== undefined && vm.modificoDatos !== null) {
                vm.modificarDistribucion = '1';
                vm.modificarDistribucionProd = '1';
                vm.modificoDatos = '0';
            }
        });

        $scope.$watch('vm.seccionactiva', function () {
            if (vm.seccionactiva !== undefined && vm.seccionactiva !== null)
                obtieneCalendario();
        ...
        function obtieneCalendario() {
            var seccionabierta = vm.listaSeccioCapitulo.find(x => x.seccion === vm.seccionactiva);
            if (seccionabierta !== undefined) {
                comunesServicio.validarCalendarioProgramacion(seccionabierta.idEntidad, seccionabierta.idNivel, seccionabierta.idSeccionCapitulo)
                    .then(function (rta) {
                        if (rta.data && (rta.statusText === "OK" || rta.status === 200)) {
                            $scope.$watch('', function () {
                                if (rta.data.Exito.toString() === 'false') {
                                    vm.mostrarMesaje = true;
                                    vm.textoMensaje = 'ESTA SECCIÓN ESTA INHABILITADA PARA EDICIÓN';
                                    habilitaBotonesSeccion(vm.seccionactiva, 'false');
                                }
                                else {
                                    vm.mostrarMesaje = false;
                                    habilitaBotonesSeccion(vm.seccionactiva, 'true');
                                }
                            });

                        }
                        else {
                            swal('', "Error al validar apertura de calendario", 'error');
                            return "false";
                        }
                    });
            }
        ...
        function habilitaBotonesSeccion(seccion, estado){
            switch (seccion) {
                case 'distribucion':
                    vm.calendarioDistribucionCuotas = estado;
                    break;
                case 'fuentes':
                    vm.calendarioFuentes = estado;
                    break;
                case 'regionalizacion':
                    vm.calendarioregionalizacion = estado;
                    break;
                case 'iniciativas':
                    vm.calendarioIniciativas = estado;
                    break;
                case 'politicastransversales':
                    vm.calendarioPoliticasTransversales = estado;
                    break;
                case 'productos':
                    vm.calendarioProductos = estado;
                    break;
                default:
                    break;
            }
        ...
        function validaCalendarioAbiertoSeccion() {
            vm.listaSeccioCapitulo.find(x => x.seccion === vm.seccionactiva);
        }
        function verificaCalendario(idEntidad, idNivel, idSeccionCapitulo, seccion) {
            var listasecciones = [];
            vm.listaSeccioCapitulo.map(function (item) {
                if (!listasecciones.includes(x => x.seccion === item.seccion))
                    listasecciones.push(item.seccion);
            });
            if (!listasecciones.includes(seccion) && idEntidad !== undefined && idNivel !== undefined
                && idSeccionCapitulo !== undefined) {
                vm.listaSeccioCapitulo.push({ idEntidad: idEntidad, idNivel: idNivel, idSeccionCapitulo: idSeccionCapitulo, seccion: seccion });
            }

            /**esto es para la carga inicial*/
            if (!listasecciones.includes('distribucion') && idEntidad !== undefined && idNivel !== undefined
                && idSeccionCapitulo !== undefined)
                    vm.seccionactiva = 'distribucion';
        ...
        //Inicio
        vm.initProgramacion = function () {
            if ($sessionStorage.InstanciaSeleccionada.tramiteId === undefined) {
                trasladosServicio.obtenerDetallesTramite($sessionStorage.InstanciaSeleccionada.numeroTramite).then(function (result) {
                    var x = result.data;
                    if (x != null) {
                        vm.tramiteId = x.TramiteId;
                        $sessionStorage.InstanciaSeleccionada.tramiteId = x.TramiteId;
                ...
                $sessionStorage.TramiteId = $sessionStorage.InstanciaSeleccionada.tramiteId;
            vm.setSessionValues();
        ...
        * */
        vm.handlerComponentes = [
            { id: 1, componente: 'distribucion', handlerValidacion: null, handlerCambios: null, esValido: true, errores: null },
            { id: 2, componente: 'fuentes', handlerValidacion: null, handlerCambios: null, esValido: true, errores: null },
            { id: 3, componente: 'regionalizacion', handlerValidacion: null, handlerCambios: null, esValido: true, errores: null },
            { id: 4, componente: 'iniciativas', handlerValidacion: null, handlerCambios: null, esValido: true, errores: null },
            { id: 5, componente: 'politicastransversales', handlerValidacion: null, handlerCambios: null, esValido: true, errores: null },
            { id: 6, componente: 'productos', handlerValidacion: null, handlerCambios: null, esValido: true, errores: null },
        ...
        vm.inicializarComponenteCheck = function (estado) {
            ...
            vm.handlerComponentesChecked = {
                'distribucion': true,
                'fuentes': true,
                'regionalizacion': true,
                'iniciativas': true,
                'politicastransversales': true,
                'productos': true
            };
        }
        ...
        vm.visualizarAlerta = function (error, errorObservacion) {
            ...
            if ($sessionStorage.Respuesta !== undefined
                && (vm.IdNivel.toLowerCase() === constantesBackbone.idNivelAprobacionProgramacion.toLowerCase() || vm.IdNivel.toLowerCase() === constantesBackbone.idNivelRevisionConcepto.toLowerCase())) {
            ...
            if ((vm.IdNivel.toLowerCase() === constantesBackbone.idNivelAprobacionProgramacion.toLowerCase() || vm.IdNivel.toLowerCase() === constantesBackbone.idNivelRevisionConcepto.toLowerCase()) && vm.siguienteDisabled) {
                vocultarDevolver = true;
            }
    ...
    angular.module('backbone').component('tramiteProgramacionRecursosPasoUno', {
        templateUrl: "src/app/formulario/ventanas/tramiteProgramacionRecursos/tramiteProgramacionRecursosPasoUno.html",
        controller: tramiteProgramacionRecursosPasoUnoController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
            registrarEvento: '&',
