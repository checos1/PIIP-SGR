(function () {
    ...
    modalAgregarDatosAdicionalesSgpController.$inject = [
        '$sessionStorage',
        '$uibModalInstance',
    ...
    function modalAgregarDatosAdicionalesSgpController(
        $uibModalInstance,
        $sessionStorage,
        gestionRecursosSGPServicio,
        utilidades
        ...
        function consultarDatos() {
            ...
            return gestionRecursosSGPServicio.obtenerDatosAdicionalesSGP(vm.idFuente, usuarioDNP, $uibModalInstance.idInstancia)
                .then(respuesta => {
                    if (!respuesta.data)
                        return;
        ...
        function obtenerTipoCofinanciador() {
            ...
            return gestionRecursosSGPServicio.obtenerTipoCofinanciador(parametros)
                .then(respuesta => {
                    if (!respuesta.data)
                        return;
        ...
        function obtenerFondo() {
            var listaFondos = [];
            return gestionRecursosSGPServicio.obtenerFondos(parametros)
                .then(respuesta => {
                    if (!respuesta.data)
                        return;
        ...
        function obtenerRubro() {
            var listaRubro = [];
            return gestionRecursosSGPServicio.obtenerRubros(parametros)
                .then(respuesta => {
                    if (!respuesta.data)
                        return;
                    vm.fondoVisible = false;
        ...
        function buscarProyecto() {
            ...
            return gestionRecursosSGPServicio.obtenerProyectos(parametros, vm.model.BpinBusqueda)
                .then(respuesta => {
                    if (!respuesta.data) {
        ...
        function guardar() {
            ...
            if (!vm.model.idtipoCofinanciador) {
                utilidades.mensajeError("Verifique el tipo de fuente de financiacion.", false);
                return false;
            }

            if (vm.fondoVisible && !vm.model.idtipoCofinanciador) {
                utilidades.mensajeError("Debe seleccionar un fondo.", false);
                return false;
            }

            if (vm.proyectoVisible && !vm.model.BpinBusqueda) {
                utilidades.mensajeError("Debe seleccionar un Bpin.", false);
                return false;
            }

            if (vm.rubroVisible && !vm.model.idRubro) {
                utilidades.mensajeError("Debe seleccionar un Rubro.", false);
                return false;
            }

            if (vm.privadoVisible && vm.model.NombrePrivado == "") {
                utilidades.mensajeError("Debe ingresar el nombre del cofinanciador privado.", false);
                return false;
            }
            if (vm.proyectoVisible) {
                if (textNombre.value.length == 0) {
                    utilidades.mensajeError("Debe validar el proyecto ingresado.", false);
                    return false;
                }                
            }
            if (vm.rubroVisible) {
                if (textNombre.value.length == 0) {
                    utilidades.mensajeError("Debe validar el rubro ingresado.", false);
                    return false;
                }
            }
            if (vm.fondoVisible) {
                if (textNombre.value.length == 0) {
                    utilidades.mensajeError("Debe seleccionar un fondo válido.", false);
                    return false;
                }
            }
            var cofinanciador = "";
            ...
            if (vm.fondoVisible) {
                ...
                var duplicate = vm.listaDatosAdicionales.some(x => x.codigoCofinanciador == textNombre.value);
            }
            if (vm.proyectoVisible) {
                codigoCofinanciador = vm.model.idtipoCofinanciador;
                ...
                var duplicate = vm.listaDatosAdicionales.some(x => x.codigoCofinanciador == textNombre.value && x.codigo == vm.model.BpinBusqueda);       
            }
            ...
            if (duplicate) {
                utilidades.mensajeError("No se puede agregar el cofinanciador seleccionado. Ya esta agregado.", false);
                return false;
            }
            ...
            gestionRecursosSGPServicio.agregarDatosAdicionalesSGP(params, usuarioDNP, $uibModalInstance.idInstancia)
        ...
        function eliminarDatoAdicional(cofinanciadorId) {
            ...
            return gestionRecursosSGPServicio.eliminarDatosAdicionalesSGP(cofinanciadorId, usuarioDNP, idInstancia)
                .then(function (response) {
                    let exito = response.data;
                    if (exito) {
                        utilidades.mensajeSuccess("Operación realizada con éxito!", false, false, false);
    ...
    angular.module('backbone').controller('modalAgregarDatosAdicionalesSgpController', modalAgregarDatosAdicionalesSgpController);
