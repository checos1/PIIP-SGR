(function () {
    ...
    resumenCostosFuentesAjustesSinTramiteSgpController.$inject = [
        'costosFuentesAjustesSinTramiteSgpServicio',
        '$sessionStorage',
        'constantesBackbone',
        '$scope',
    ];

    function resumenCostosFuentesAjustesSinTramiteSgpController(
        costosFuentesAjustesSinTramiteSgpServicio,
        $sessionStorage,
        constantesBackbone,
        $scope,
        ...
        function obtenerCostosPIIPvsFuentesPIIP() {
            ...
            return costosFuentesAjustesSinTramiteSgpServicio.obtenerCostosPIIPvsFuentesPIIP($sessionStorage.idInstancia, vm.idUsuario, vm.idInstancia).then(
                function (respuesta) {
                            ...
                            switch (arregloDatosEtapas[ls].Etapa) {
                                case 'Operación':
                                    {                                                                                
                                        ...
                                        if (total_opeAjuste > 0 || total_opeCostos > 0) {
                                            vm.tipoOperacion = 3;
                                            vm.operacion = true;
                                            if (total_opeAjuste != total_opeCostos) {
                                                vm.errores.operacion = true;
                                            } else vm.errores.operacion = false;
                                        } else vm.errores.operacion = false;
                                        break;
                                    }
                                case 'Preinversión':
                                    {                                        
                                        ...
                                        resumenSubtotal_prei = {
                                            ...
                                            errorOpe: preinversion
                                        }
                                        if (total_preiAjuste > 0 || total_preiCostos) {
                                            vm.tipoOperacion = 1;
                                            vm.preinversion = true;
                                            if (total_preiAjuste != total_preiCostos) {
                                                vm.errores.preinversion = true;
                                            } else vm.errores.preinversion = false;
                                        } else vm.errores.preinversion = false;
                                        break;
                                    }
                                case 'Inversión':
                                    {                                        
                                        ...
                                        if (total_invAjuste > 0 || total_invCostos > 0) {
                                            vm.tipoOperacion = 2;
                                            vm.inversion = true;
                                            if (total_invAjuste != total_invCostos ) {
                                                vm.errores.inversion = true;
                                            } else vm.errores.inversion = false;                                            
                                        } else vm.errores.inversion = false;
                                        break;
                                    }
                            }
                        }
                        vm.mostrarTab(vm.tipoOperacion);
                    } else {
    ...
    angular.module('backbone').component('resumenCostosFuentesAjustesSinTramiteSgp', {
        templateUrl: "src/app/formulario/ventanas/SGP/ajustesProyectoSGP/componentes/recursosSGP/costoActividades/ResumenCostosFuentesAjustes/resumenCostosFuentesAjustesSinTramiteSgp.html",
        controller: resumenCostosFuentesAjustesSinTramiteSgpController,
        controllerAs: "vm",
        bindings: {
            disabled: '=',
            callback: '&',
