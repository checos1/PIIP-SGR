(function () {
    ...
    informacionPresupuestalController.$inject = [
        '$scope',
        '$sessionStorage',
    ...
    function informacionPresupuestalController(
        $scope,
        $sessionStorage,
        tramiteVigenciaFuturaServicio,
        utilidades,
        ...
        vm.handlerComponentes = [
            { id: 1, componente: 'informacionpresupuestalconpes', handlerValidacion: null, handlerCambios: null, esValido: true },
            { id: 2, componente: 'informacionpresupuestalcdp', handlerValidacion: null, handlerCambios: null, esValido: true },
            { id: 3, componente: 'informacionpresupuestalresumen', handlerValidacion: null, handlerCambios: null, esValido: true },
            { id: 4, componente: 'informacionpresupuestalrp', handlerValidacion: null, handlerCambios: null, esValido: true }
        ];
        ...
        vm.validaAplicaConpes = function () {
            if (vm.tipotramiteid == 18 && vm.idNivel.toUpperCase() === constantesBackbone.idNivelElaboracionConcepto) {
                vm.showConpes = false;
            }
            else {
                tramiteVigenciaFuturaServicio.validarVigenciaConpes(vm.tramiteid)
                    .then(function (response) {
                        if (response?.data?.Estado) {
                            vm.showConpes = response.data.Data;
                            //Esta validacion se coloca para el tramite VFexcecionales en el paso 3
        ...
        function MostrarCRP() {
            if (vm.tipotramiteid == 5) {
                vm.mostrarCRP = true;
            }
        }

        function MostrarCDP() {
            if (vm.tipotramiteid == 18) {
                vm.MostrarCDP = false;
            }
        }
       ...
       * */
        ...
        vm.inicializarComponenteCheck = function () {
            vm.handlerComponentesChecked = {
                ...
                'informacionpresupuestalresumen': true,
            };
        }
    ...
    angular.module('backbone').component('informacionPresupuestal', {
        templateUrl: "src/app/formulario/ventanas/tramiteVigenciaFutura/componentes/informacionPresupuestal/informacionPresupuestal.html",
        controller: informacionPresupuestalController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
            tramiteid: '@',
