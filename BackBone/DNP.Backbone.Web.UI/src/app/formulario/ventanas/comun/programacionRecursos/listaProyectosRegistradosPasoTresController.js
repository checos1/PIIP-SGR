(function () {
    ...
    listaProyectosRegistradosPasoTresController.$inject = [
        '$scope',
        '$sessionStorage',
        'utilidades',
        'comunesServicio',
    ...
    function listaProyectosRegistradosPasoTresController(
        $scope,
        $sessionStorage,
        utilidades,
        comunesServicio,
        ...
        $scope.pages = [];
        vm.abrirModalAgregarProyecto = abrirModalAgregarProyecto;


        function abrirModalAgregarProyecto() {

            $uibModal.open({
                templateUrl: 'src/app/formulario/ventanas/tramiteProgramacionRecursos/Componentes/inclusionProyectosPr/modal/modalAgregarProyecto.html',
                controller: 'modalAgregarProyectoController',
        ...
        $scope.$watch('vm.section', function () {
            ...
            switch (vm.section) {
                ...
                case 'iniciativas':
                    ...
                    vm.mostrarpoliticas = false;
                    break;
                case 'politicastransversalesaprobado':
        ...
        function GuardarValores(datos) {
            ...
            angular.forEach(datos.Proyectos, function (series) {
                if ((series.NacionCSFAC > 0 && series.NacionCSFA == 0) || (series.NacionSSFAC > 0 && series.NacionSSFA == 0) || (series.DCCAC > 0 && series.DCCA == 0)) {
                    if (series.NacionCSFA == 0 && series.NacionSSFA == 0 && series.DCCA == 0) {
                        cambioCero++;
                    }
                }
                let valores = {
                    ProyectoId: series.ProyectoId,
                    DistribucionCuotaComunicadaNacionCSF: series.NacionCSFA,
                    DistribucionCuotaComunicadaNacionSSF: series.NacionSSFA,
                    DistribucionCuotaComunicadaPropios: series.DCCA
                };
                            ...
                            if (respuesta.data && (respuesta.statusText === "OK" || respuesta.status === 200)) {
                                if (respuesta.data.Exito) {
                                    ...
                                    utilidades.mensajeSuccess("Verifique que los datos no presenten inconsistencias:<br/>" +
                                        ...
                                        " En la tabla de proyectos, los recursos aprobados por proyecto deben ser mayores o iguales a su vigencia futura.", false, false, false, "Los datos han sido guardados con éxito");
                                    datos.EAjCT = false;
                                    vm.modificodatos = '1';
                                    vm.init();
                                    vm.limpiarErrores();
        ...
        function eliminarProyecto(proyecto) {
            var a = proyecto;
            // utilidades.mensajeError('Eliminar proyecto.');
            ...
            utilidades.mensajeWarning("Es posible que se afecten los datos y la tabla de resumen."
                + " ¿Esta seguro de continuar?",
                function funcionContinuar() {

                    return tramiteProgramacionRecursosServicio.BorrarTramiteProyecto(Programacion).then(
                        function (respuesta) {
                            if (respuesta.data && (respuesta.statusText === "OK" || respuesta.status === 200)) {
                                if (respuesta.data.Exito) {
                                    //guardarCapituloModificado();
                ...
                "Aceptar",
                "Cancelar",
                "El proyecto seleccionado va a ser eliminado de la tabla."
            )
        ...
        vm.actualizaFila = function (event, datos, campo, proyectoId) {
            ...
            if (campo === 'NacionCSFAP') datos.Proyectos.find(p => p.ProyectoId == proyectoId).NacionCSFA = event.target.value;
            if (campo === 'NacionSSFAP') datos.Proyectos.find(p => p.ProyectoId == proyectoId).NacionSSFA = event.target.value;
            if (campo === 'DCCAP') datos.Proyectos.find(p => p.ProyectoId == proyectoId).DCCA = event.target.value;

            angular.forEach(datos.Proyectos, function (series) {

                series.TDCCA = parseFloat(series.NacionCSFA) + parseFloat(series.NacionSSFA) + parseFloat(series.DCCA);
    ...
    angular.module('backbone').component('listaProyectosRegistradosPasoTres', {

        templateUrl: "src/app/formulario/ventanas/comun/programacionRecursos/listaProyectosRegistradosPasoTres.html",
        controller: listaProyectosRegistradosPasoTresController,
        controllerAs: "vm",
        bindings: {
            guardadoevent: '&',
            notificacioncambios: '&',
