(function() {
    ...
    agregarCdpProyectoController.$inject = [
        '$scope',
        'utilidades',
        'tramiteSGPServicio',
        '$uibModal',
        'cargaMasivaCdpService',
        '$sessionStorage',
        'justificacionCambiosServicio'
    ];

    function agregarCdpProyectoController(
        $scope,
        utilidades,
        tramiteSGPServicio,
        $uibModal,
        ...
        vm.setEvents = function () {
            ...
            $scope.$watch('vm.errores', function () {
                if (vm.errores != undefined && vm.errores !== null && vm.errores !== '' && vm.errores !== []) {
                    ...
                    if (errores["informacionpresupuestalcdp"] != undefined && errores["informacionpresupuestalcdp"] != null &&
                        errores["informacionpresupuestalcdp"].length > 0) {
                        vm.cdpListError = errores["informacionpresupuestalcdp"].filter(p => p.proyectoid == vm.proyectoid);
        ...
        // Carga los CDP existentes por proyecto id
            ...
            if (vm.tramiteid != undefined && vm.proyectoid != undefined) {
                tramiteSGPServicio.obtenerProyectoRequisitosPorTramite(vm.proyectoid, vm.tramiteid)
                    .then(function (response) {
                        ...
                        utilidades.mensajeError('No fue posible consultar los CDP asociados al proyecto');
                    });
            }
        }

        // Mapea el listado de CDP recibido desde el back a un objeto procesable desde el front
        vm.convertirModeloACdp = function(response) {
            vm.cdpListRaw = response.data;

            vm.cdpList = response.data.map((cdp) => {
                const listaValoresCdp = cdp.ListaTiposRequisito.find(w => w.Descripcion == 'Documento CDP').ListaValores;
                ...
                return {
                    ...
                    NumeroContrato: cdp.NumeroContrato,
                    Tipo: 1,
        ...
        // Lanza el modal de creación/ edición de CDP
        vm.crearCdp = function (editarCdpId = 0) {
            var templateUrl = "src/app/formulario/ventanas/comun/certificadoDisponibilidadPresupuestal/componentes/agregarCdpProyectoModal/agregarCdpProyectoModal.html";

            try {
                let modalInstance = $uibModal.open({
                    templateUrl: templateUrl,
                    controller: 'agregarCdpProyectoModalController',
                    controllerAs: "vm",
        ...
        // Remueve un CDP del listado y actualiza la información de CDP por proyecto
        ...
        // Carga la plantilla de ejemplo para carga masiva de CDP
        vm.generarPlantillaEjemplo = function () {
            cargaMasivaCdpService.generarDocumentoEjemploCdp();
        }
        ...
        vm.validarArchivo = function () {
            ...
            cargaMasivaCdpService.procesarArchivoCdp(vm.files[0])
                .then(function (response) {
                    const modelResponse = cargaMasivaCdpService.validarArchivo(response, vm.cdpList);
        ...
        vm.cargarArchivo = function () {
            ...
            cargaMasivaCdpService.procesarArchivoCdp(vm.files[0])
                .then(function (response) {
                    const modelList = cargaMasivaCdpService.convertirRespuestaAModelo(response, vm.tramiteid, vm.proyectoid, vm.rolid);
    ...
    angular.module('backbone').component('agregarCdpProyecto', {
        templateUrl: "src/app/formulario/ventanas/comun/certificadoDisponibilidadPresupuestal/componentes/agregarCdpProyecto/agregarCdpProyecto.html",
        controller: agregarCdpProyectoController,
        controllerAs: "vm",
        bindings: {
            proyectoid: '@',
            tramiteid: '@',
