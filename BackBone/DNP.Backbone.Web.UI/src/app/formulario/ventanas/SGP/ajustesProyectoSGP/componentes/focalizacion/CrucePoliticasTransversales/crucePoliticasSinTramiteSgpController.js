(function () {
    ...
    crucePoliticasSinTramiteSgpController.$inject = ['$scope', '$sessionStorage', '$uibModal', 'utilidades',
        'constantesBackbone', '$timeout', 'focalizacionAjustesSGPServicio', 'transversalSgpServicio'
    ];

    function crucePoliticasSinTramiteSgpController(
        $scope,
        $sessionStorage,
        $uibModal,
        utilidades,
        ...
        var vm = this;
        vm.init = init;
        vm.nombreComponente = "focalizacionpolsgpcrucepoliticassintramitesgp";
        ...
        vm.ERRORCRUCEAJU = function (errores) {
            ...
            valores.forEach(v => {
                ...
                var seccionpd = document.getElementById("errorcrucepfuentepplocpdv-" + err[0] + "-" + err[1] + "-" + err[2] + "-" + err[3] + "-" + err[4]);
                if (seccionpd != undefined) {
                    seccionpd.classList.remove('hidden');
                }
            });
    ...
    angular.module('backbone').component('crucePoliticasSinTramiteSgp', {
        templateUrl: "/src/app/formulario/ventanas/SGP/ajustesProyectoSGP/componentes/focalizacion/CrucePoliticasTransversales/crucePoliticasSinTramiteSgp.html",        
        controller: crucePoliticasSinTramiteSgpController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
