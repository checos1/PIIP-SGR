(function () {
    ...
    datosVerificacionCtusISgrController.$inject = ['$scope', 'utilidades','$sessionStorage'];

    function datosVerificacionCtusISgrController(
        $scope,
        utilidades,
        $sessionStorage        
    ) {
        var vm = this;
        vm.nombreComponente = "sgrctusintegradoelaboraciondatosverificacion";
        vm.nombreComponenteHijo = "sgrctusintegradoelaboraciondatosverificacionagregarsectores";
        ...
        vm.handlerComponentes = [
            { id: 1, componente: 'sgrctusintegradoelaboraciondatosverificacionagregarsectores', handlerValidacion: null, handlerCambios: null, esValido: true },
            { id: 2, componente: 'sgrctusintegradoelaboraciondatosverificaciondatosadicionalesverificacion', handlerValidacion: null, handlerCambios: null, esValido: true }

        ];
        vm.handlerComponentesChecked = {};

        vm.componentesRefreshSector = [
            "sgrctusintegradoelaboraciondatosverificaciondatosadicionalesverificacion"
        ];
        ...
        vm.guardado = function (nombreComponenteHijo) {
            vm.callback();
            if (nombreComponenteHijo == "sgrctusintegradoelaboraciondatosverificaciondatosadicionalesverificacion") {
                vm.guardadocomponent({ nombreComponente: vm.nombreComponente, nombreComponenteHijo: nombreComponenteHijo });
                vm.notificarrefrescoSector();
            }

            if (nombreComponenteHijo == "sgrctusintegradoelaboraciondatosverificacionagregarsectores") {
                vm.guardadocomponent({ nombreComponente: vm.nombreComponente, nombreComponenteHijo: nombreComponenteHijo });
            }
        }
        ...
        vm.notificarRefresco = function (handler, nombreComponente) {
            if (nombreComponente == "sgrctusintegradoelaboraciondatosverificacionagregarsectores" ) {
        ...
        vm.inicializarComponenteCheck = function () {
            vm.handlerComponentesChecked = {
                'sgrctusintegradoelaboraciondatosverificacionagregarsectores': true,
                'sgrctusintegradoelaboraciondatosverificaciondatosadicionalesverificacion': true
            };
        }
    ...
    angular.module('backbone').component('datosVerificacionCtusISgr', {
        templateUrl: "/src/app/formulario/ventanas/SGR/ctusIntegrado/elaboracion/datosVerificacion/datosVerificacionCtusISgr.html",

        controller: datosVerificacionCtusISgrController,
        controllerAs: "vm",
        bindings: {
            callback: '&',
